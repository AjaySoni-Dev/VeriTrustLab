...
VeriTrust Lab SMTP Enforcement Gateway (long-running Node process)
        |
        | received RFC 822 message + gateway-owned Received header + observed SMTP facts over HTTPS
        v
...
That means the decision can reuse the existing MailGraph stack: bounded MIME parsing; analysis-input hashing and temporary storage; MailGuard phishing evidence; deterministic forensic content observations; Swift child-URL analysis; SPF at the trusted receiver boundary; DKIM/DMARC/ARC evidence; sender-identity relationships; Received-hop extraction and approximate infrastructure context; attachment metadata/hash intelligence; available evidence persistence; Gateway correlation v3; policy enforcement; reports, reviews and cases.

The relay does not invent a second scoring system. The relay uses the existing VeriTrust Lab policy recommendation as its decision input. The transport layer does not rescore the message; it maps that recommendation into configured SMTP behavior.
...
The bundled runtime logging calls record operational identifiers/status values rather than message bodies or configured credentials; deployers should preserve that property when adding logging or observability. Sender-supplied `X-VeriTrust-*` headers are removed before relay, and gateway-generated decision headers are then added before relay. Analysis runs on the gateway-augmented message, including the receiver-owned `Received:` header, before those spoofable headers are stripped for downstream delivery, so supplied spoof attempts remain visible to the analysis pipeline.
...
If VeriTrust Lab returns `quarantine` or `block`, the send command receives an SMTP error and Laptop B receives nothing. If the message passes with `allow` or `warn`, the downstream receiver gets the message and the saved raw email contains the gateway's `Received:` boundary plus gateway-added `X-VeriTrust-*` decision headers.
...
Use `/gateway-powershell#live-smtp` for the operational guide, including the server-side receiver secret and trusted authserv requirements. For a simple demonstration, place both laptops in the same Tailscale tailnet (the same Tailscale account is the easiest option).


## Persistent PowerShell CLI distribution

The repository also ships the bundled VeriTrust Lab persistent CLI used by the public `/gateway-powershell#live-smtp` guide. It is an operator-facing wrapper around this SMTP runtime, not a second scoring engine.
