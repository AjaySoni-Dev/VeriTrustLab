...
if (isModuleEnabled('gateway')) {
  ...
  if (evidenceKey) {
    ...
    if (productionLike) failures.push('VERITRUST_EVIDENCE_SIGNING_PRIVATE_KEY is required for production Evidence Passport signing');
    else warnings.push('Evidence Passport signing is using the purpose-separated development/test fallback derived from VERITRUST_CONTENT_HMAC_KEY; production requires a dedicated Ed25519 key');
  }
