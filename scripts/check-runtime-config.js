...
const failures = [];
const warnings = [];
const productionRuntime = process.env.NODE_ENV === 'production' || process.env.VERCEL_ENV === 'production' || process.env.VERCEL === '1';
...
if (isModuleEnabled('gateway')) {
  ...
  if (evidenceKey) {
    ...
    failures.push('VERITRUST_EVIDENCE_SIGNING_PRIVATE_KEY is required in production so Evidence Passport signing has an isolated, rotatable Ed25519 key');
  } else {
    warnings.push('Evidence Passport signing is using the purpose-separated development fallback derived from VERITRUST_CONTENT_HMAC_KEY; configure a dedicated Ed25519 key before production');
  }
  ...
  const geoProvider = String(getOptionalEnv('VERITRUST_GEO_PROVIDER', 'ipwhois')).trim().toLowerCase();
  if (['configured', 'custom', 'proxy'].includes(geoProvider) && !getOptionalEnv('VERITRUST_GEO_PROVIDER_URL', '')) {
    failures.push('VERITRUST_GEO_PROVIDER_URL is required when VERITRUST_GEO_PROVIDER uses the configured/custom/proxy mode');
  }
  if (productionRuntime && ['ipwhois', 'ipwho.is'].includes(geoProvider)) {
    warnings.push('GeoTrace is using the public ipwho.is fallback in production; configure a production geo provider/proxy to avoid shared-egress quota exhaustion');
  }
