...
test('All four SIH USP surfaces and release contracts remain wired into the shipped UI/API/schema', () => {
  ...
  const dashboardUi = fs.readFileSync(require.resolve('../assets/js/pages/dashboard.js'), 'utf8');
  ...
  assert.match(ui, /data-usp-panel="progressive"/u);
  assert.match(ui, /data-usp-panel="geotrace"/u);
  assert.match(ui, /data-usp-panel="campaign"/u);
  assert.match(ui, /data-usp-panel="passport"/u);
  assert.match(ui, /campaignMemory\.common_entities/u);
  assert.match(ui, /item\.weight/u);
  assert.match(ui, /lineage\.parent_scan_id/u);
  assert.match(ui, /\/api\/v2\/phishing\/export\/\$\{encodeURIComponent\(scanId\)\}/u);
  assert.match(dashboardUi, /isEmailForensicScan/u);
  assert.match(dashboardUi, /data-scan-action="open-email"/u);
