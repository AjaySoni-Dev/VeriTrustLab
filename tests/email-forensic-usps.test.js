...
test('Evidence Passport verifies unchanged evidence and rejects tampering', () => {
  ...
  const valid = verifyEvidencePackage({ passport, evidence: packaged }, { trustedKeyIds: trustedEvidenceKeyIds() });
  ...
  const invalid = verifyEvidencePackage({ passport, evidence: tampered }, { trustedKeyIds: trustedEvidenceKeyIds() });
...
test('All four SIH USP surfaces and release contracts remain wired into the shipped UI/API/schema', () => {
  ...
  const hardeningMigration = fs.readFileSync(require.resolve('../supabase/migrations/20260914_evidence_passport_hardening.sql'), 'utf8');
  ...
  assert.match(verifierPage, /Issuer trust stays separate/u);
  assert.match(verifierPage, /evidence-passport-browser\.js/u);
  ...
  assert.match(hardeningMigration, /evidence_payload jsonb/iu);
  assert.match(hardeningMigration, /foreign key \(scan_id, org_id\)/iu);
