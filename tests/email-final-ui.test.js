...
test('final email UI remains scrollable, exposes Evidence Passport actions, and opens the complete PDF report', () => {
  ...
  assert.match(ui, /Evidence integrity/u);
  assert.match(ui, /Export signed Evidence JSON/u);
  assert.match(ui, /Export reconstructed JSON \(unsigned\)/u);
  assert.doesNotMatch(css, /body\.vt-email-investigation\s*\{[\s\S]*?height:\s*100vh;[\s\S]*?overflow:\s*hidden;/u);
  assert.match(css, /analysis-report \{ max-height:none; overflow:visible;/u);
