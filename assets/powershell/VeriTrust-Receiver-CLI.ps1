...
$ApiBaseUrl = if ($env:VERITRUST_API_BASE_URL) { $env:VERITRUST_API_BASE_URL.TrimEnd('/') } else { 'https://www.veritrustlab.in' }
...
function Write-BrandHeader {
    ...
    $meta=@(
        ...
        (A $Role '148;148;148'),
        (A 'Trusted SMTP · Forensic analysis · Policy enforcement' '120;120;120'),
...
function Show-MailCard {
    ...
    Write-Line '';Write-Rule '73;210;126';Write-Tag 'RELAYED' 'Message forwarded under the current VeriTrust Lab policy decision.' 'ok'
...
$gatewayScript=Join-Path $RuntimeRoot 'server.js';$sinkScript=Join-Path $RuntimeRoot 'test-receiver.js'
...
try {
    ...
    $sinkProcess=Start-NodeProcess $node $sinkScript $RuntimeRoot $sinkEnv;Start-Sleep -Milliseconds 700;if($sinkProcess.HasExited){throw 'Local downstream receiver exited during startup.'};Write-Tag 'OK' ('Downstream receiver: 127.0.0.1:{0}' -f $SinkPort) 'ok'
    ...
    Write-Line 'Gateway is live. EML files forwarded to the local demo receiver appear here.' DarkGray
