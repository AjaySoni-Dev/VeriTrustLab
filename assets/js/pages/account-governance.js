(function accountGovernance(global) {
  ...
  function renderRequests(requests) {
    ...
    if (!root) return;
    if (!requests.length) {
      root.innerHTML = '<div class="model-card-empty"><strong>No active requests</strong><p>Your data-export and erasure-review requests will appear here.</p></div>';
      return;
    }
  ...
  function renderPolicy(policy) {
    ...
    root.innerHTML = `
      ...
      <div><span>Automated deletion</span><strong>${policy.retention_enforcement_enabled ? 'Configured enabled' : 'Configured disabled'}</strong></div>
    `;
  }
  ...
  async function load() {
    ...
    const status = document.querySelector('[data-privacy-status]');
    if (status) { status.textContent = 'Policy loaded'; status.className = 'status-pill safe'; }
