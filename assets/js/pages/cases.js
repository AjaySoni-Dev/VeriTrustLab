(function caseWorkspace(global) {
  ...
  const outcomeLabel = (value) => {
    ...
    if (normalized === 'safe') return 'No confirmed threat';
  ...
  const isEmailForensicEvidence = (item = {}) => {
    const provenance = item.provenance || {};
    const source = String(provenance.source || provenance.scan_source || '').toLowerCase();
    const mode = String(provenance.input_mode || '').toLowerCase();
    return provenance.evidence_schema === 'phishing-evidence-7'
      || ['phishing-v2', 'trusted-receiver-v2'].includes(source)
      || ['plain_text', 'raw_eml', 'trusted_receiver_event'].includes(mode);
  };

  const sourceInvestigationLink = (item = {}) => {
    const scanId = String(item.source_id || item.provenance?.scan_id || '').trim();
    if (!isEmailForensicEvidence(item) || !/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(scanId)) return '';
    return `<a class="case-source-investigation" href="/phishing-result?scan_id=${encodeURIComponent(scanId)}">Open source email investigation →</a>`;
  };
  ...
  function renderCaseList(cases) {
    ...
    if (!rows.length) {
      target.innerHTML = '<div class="empty-state-table"><strong>No email cases match this queue.</strong><p>Change the filters or run a new email investigation. Review cases appear when the deployed workflow creates case records; not every completed investigation necessarily creates a case.</p></div>';
      return;
    }
    target.innerHTML = `<div class="case-queue">${rows.map((item) => {
      ...
      return `<a class="case-queue-item" href="case.html?id=${encodeURIComponent(item.id)}">
        ...
        <div><span class="case-meta">Current outcome</span><strong>${escapeHtml(outcomeLabel(decision.outcome || 'pending'))}</strong></div>
  ...
  async function loadCaseList(filters = {}) {
    setStatus('Loading the email investigation case queue.');
    ...
    const emailCases = (Array.isArray(payload.cases) ? payload.cases : [])
      .filter((item) => item?.investigation_type === 'email');
    renderCaseList(emailCases);
    setStatus(`${emailCases.length} email investigation case${emailCases.length === 1 ? '' : 's'} in this view.`);
  }
  ...
  function renderDecisions(caseRow) {
    ...
    target.innerHTML = caseRow.decisions.slice().reverse().map((item) => `<article class="case-decision-card" data-kind="${escapeHtml(item.decision_kind)}">
      <div class="case-chip-row"><span class="case-chip">${escapeHtml(titleCase(item.decision_kind))}</span><span class="case-chip">${escapeHtml(outcomeLabel(item.outcome))}</span>${riskPill(item.risk_level)}</div>
