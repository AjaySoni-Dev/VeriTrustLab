(function evidenceVerifyPage(global) {
  ...
  function renderResult(result) {
    ...
    const checks = [
      ...
      ['Evidence SHA-256', result.evidence_hash_valid, result.evidence_hash_valid ? 'The supplied evidence matches the signed evidence hash.' : 'The supplied evidence differs from the signed hash.'],
      ['Manifest SHA-256', result.manifest_hash_valid, result.manifest_hash_valid ? 'The supplied manifest matches the signed manifest hash.' : 'The evidence manifest differs from the signed hash.'],
    ...
    root.innerHTML = `<div class="verify-result-header"><span>Verification result</span><h2>${result.valid ? 'Signed package checks passed' : 'Verification failed'}</h2><p>${result.valid ? 'The supplied evidence matches the signed hashes, the Ed25519 signature is valid, and the signing key is trusted by this deployment.' : 'One or more package-integrity or issuer-trust checks failed. Do not treat this package as matching the signed VeriTrust Lab record.'}</p></div>
      ...
      <p>This verifies signed package consistency and configured issuer trust only. It does not establish content truth, sender identity, attribution, legal admissibility, or message safety.</p>
      ${(() => { const scanId = new URLSearchParams(global.location.search).get('scan_id'); return UUID_PATTERN.test(scanId || '') ? `<p class="verify-return"><a href="/phishing-result?scan_id=${encodeURIComponent(scanId)}">← Back to investigation</a></p>` : ''; })()}`;
    root.hidden = false;
    root.focus();
  }
