(function modelPerformance(global) {
  ...
  async function load() {
    ...
    if (!cards.length) {
      summary.innerHTML = '<strong>Published validation not available</strong><p>No approved, published model card was returned for the currently exposed model registry. VeriTrust Lab therefore displays no benchmark claim.</p>';
  ...
  load().catch((error) => {
    ...
    root.innerHTML = '<div class="model-card-empty"><strong>No published model card is available</strong><p>No benchmark metric is shown without a published supporting record.</p></div>';
  });
})(window);
