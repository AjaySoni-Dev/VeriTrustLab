document.addEventListener('DOMContentLoaded', async () => {
  ...
  const renderScans = () => {
    ...
    if (!emailScans.length) {
      table.innerHTML = '<div class="empty-state-table"><strong>No email investigations yet.</strong><p>Start with pasted text, upload a raw EML export, or use the trusted SMTP receiver.</p></div>';
      return;
    }
    if (!matches.length) {
