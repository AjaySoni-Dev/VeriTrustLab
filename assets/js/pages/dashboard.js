document.addEventListener('DOMContentLoaded', async () => {
  ...
  const isEmailForensicScan = (scan = {}) => {
    ...
    return metadata.evidence_schema === 'phishing-evidence-7'
  ...
  const scanTypeLabelForScan = (scan = {}) => isEmailForensicScan(scan) ? 'Email Investigation' : scanTypeLabel(scan.scan_type);
  ...
  const filteredScans = () => cachedScans.filter((scan) => {
    ...
    if (activeScanFilter === 'phishing') return scan.scan_type === 'phishing' || isEmailForensicScan(scan);
    if (activeScanFilter === 'link') return scan.scan_type === 'link';
    if (activeScanFilter === 'gateway') return scan.scan_type === 'gateway' && !isEmailForensicScan(scan);
  ...
  const summarizeScans = (scans) => {
    ...
    const phishing = scans.filter((scan) => scan.scan_type === 'phishing' || isEmailForensicScan(scan)).length;
    const gateway = scans.filter((scan) => scan.scan_type === 'gateway' && !isEmailForensicScan(scan)).length;
  ...
  function renderScans(scans) {
    ...
    const emailForensic = isEmailForensicScan(scan);
    const gateway = scan.scan_type === 'gateway' ? (scan.metadata?.gateway || {}) : null;
    const detailIdentity = emailForensic
      ? `Email evidence stage: ${escapeHtml(emailForensicStage(scan))} &middot; Source: ${escapeHtml(titleCase(gateway?.source || scan.source || 'phishing-v2'))} &middot; Scan ID: ${escapeHtml(scan.id)}`
      : gateway
    ...
    const typeMarkup = emailForensic
      ? `<span class="scan-type-with-stage"><strong>Email Investigation</strong><small>${escapeHtml(emailForensicStage(scan))}</small></span>`
      : escapeHtml(scanTypeLabelForScan(scan));
    const actionMarkup = emailForensic
      ? `<button type="button" data-scan-action="open-email" data-scan-index="${index}">Open Investigation</button><button type="button" data-scan-action="email-json" data-scan-index="${index}">Evidence JSON</button>`
      : `<button type="button" data-scan-action="download" data-scan-index="${index}">JSON Report</button><button type="button" data-scan-action="print" data-scan-index="${index}">Save PDF Report</button>`;
    return `
          <div class="scan-row scan-row-rich" role="row">
            <span data-label="Type">${typeMarkup}</span>
            ...
            <span class="scan-actions" data-label="Reports">${actionMarkup}</span>
          </div>
          <details class="scan-detail-row">
            <summary>${gateway && !emailForensic ? 'View full details' : 'View details'}</summary>
            <p>${escapeHtml(result.explanation || 'Saved scan result.')}</p>
            ${gateway && !emailForensic
    ...
    table.querySelectorAll('[data-scan-action]').forEach((button) => {
      button.addEventListener('click', () => {
        ...
        if (button.dataset.scanAction === 'open-email') globalThis.location.href = `/phishing?scan_id=${encodeURIComponent(scan.id)}`;
