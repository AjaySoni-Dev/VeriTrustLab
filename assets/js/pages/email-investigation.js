(function emailInvestigationPage(global) {
  ...
  function renderCompactUspSummary({ evidence, infrastructure, infrastructureSummary, campaignMemory, passport, scanId }) {
    ...
    const publicHopCount = Number.isFinite(Number(infrastructureSummary.public_hop_count))
      ? Number(infrastructureSummary.public_hop_count)
      : publicHops.length;
    ...
    const campaignState = campaignMemory.state === 'CORRELATED'
      ? `${relatedCount} prior investigation${relatedCount === 1 ? '' : 's'} linked`
      : campaignMemory.state === 'UNAVAILABLE'
        ? 'Correlation unavailable'
        : 'No strong prior campaign match';
    const geoState = trusted.length
      ? `${trusted.length} trusted SMTP boundar${trusted.length === 1 ? 'y' : 'ies'} observed`
      : publicHopCount
        ? `${publicHopCount} public relay claim${publicHopCount === 1 ? '' : 's'} · origin unverified`
        : 'No public relay boundary available';
    ...
    const nextAction = next?.action === 'UPLOAD_ORIGINAL_EML'
      ...
      : next?.action === 'USE_TRUSTED_RECEIVER'
        ? '<a class="email-usp-link" href="/gateway-powershell#live-smtp">Acquire trusted SMTP evidence →</a>'
        : '<span class="email-usp-complete">Highest supported stage reached</span>';
    ...
    return `<section class="email-usp-summary" aria-label="Core VeriTrust forensic capabilities">
      ...
      <article class="email-usp-card">
        ...
        <div><span>MailGraph Campaign Memory™</span><strong>${escapeHtml(campaignState)}</strong><small>${campaignMemory.campaign_id ? `Campaign ${escapeHtml(campaignMemory.campaign_id)}` : 'Conservative correlation requires sufficiently strong repeated forensic entities.'}</small></div>
      </article>
      <article class="email-usp-card">
  ...
  function renderProgressiveEvidenceDetails(evidence, scanId) {
    ...
    const dimensionLabels = {
      content: 'Content', ai_model: 'AI model', authentication: 'Authentication', identity: 'Identity', links: 'Links',
      attachments: 'Attachments', infrastructure: 'Infrastructure', threat_intelligence: 'Threat intelligence',
    };
    ...
    const matrix = Object.entries(dimensions).map(([key, value]) => `<div class="email-evidence-dimension"><span>${escapeHtml(dimensionLabels[key] || titleCase(key))}</span><strong data-coverage="${escapeHtml(String(value || 'UNKNOWN').toLowerCase())}">${escapeHtml(friendlyStatus(value || 'UNKNOWN'))}</strong></div>`).join('');
    ...
    const nextMarkup = nextActions.length ? nextActions.map((item) => {
      ...
      const action = item.action === 'UPLOAD_ORIGINAL_EML'
        ? `<button class="btn btn-secondary" type="button" data-upgrade-evidence="eml" data-parent-scan="${escapeHtml(scanId || '')}">Add original .eml</button>`
    ...
    const parent = lineage.parent_scan_id
      ? `<div class="email-lineage"><span>Investigation lineage</span><strong>Upgraded from a prior evidence stage</strong><a href="/phishing?scan_id=${encodeURIComponent(lineage.parent_scan_id)}">Open previous scan ${escapeHtml(String(lineage.parent_scan_id).slice(0, 8))}… →</a></div>`
    ...
    return `<details class="email-forensic-panel" data-usp-panel="progressive" open>
      <summary><span class="email-forensic-index">01</span><span><strong>Progressive Evidence Escalation™</strong><small>See exactly what this acquisition stage could and could not establish.</small></span><em>${escapeHtml(evidenceStageName(evidence.input_mode))}</em></summary>
      <div class="email-forensic-body">
        <div class="email-ladder-track">${stageMarkup}</div>
        <p class="email-ladder-note">${escapeHtml(completeness.wording || 'Evidence completeness describes forensic coverage, not threat risk.')}</p>
  ...
  function renderGeoTraceDetails(infrastructure, infrastructureSummary) {
    ...
    const map = points.length
      ? `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Approximate mail infrastructure coordinate plot"><g class="email-geotrace-grid">${grid}</g>${polyline}${nodes}</svg>`
    ...
    const hopRows = allHops.length ? allHops.map((hop, index) => {
      ...
      const infra = [hop.host, hop.ip_address, hop.asn ? `AS${String(hop.asn).replace(/^AS/iu, '')}` : '', hop.asn_org].filter(Boolean).join(' · ') || 'No infrastructure identity recorded';
      const trusted = hop.trust_level === 'trusted_receiver';
      return `<article class="email-hop-row" data-trust="${trusted ? 'trusted' : 'observed'}"><span>${Number(hop.hop_index ?? index) + 1}</span><div><strong>${escapeHtml(infra)}</strong><small>${escapeHtml(location)} · ${escapeHtml(titleCase(hop.ip_classification || 'unknown'))}</small></div><em>${trusted ? 'Trusted receiver' : 'Observed relay'}</em></article>`;
    ...
    return `<details class="email-forensic-panel" data-usp-panel="geotrace"${points.length ? ' open' : ''}>
      <summary><span class="email-forensic-index">02</span><span><strong>Trust-Boundary GeoTrace™</strong><small>Reconstructed relay context stays distinct from directly observed SMTP evidence.</small></span><em>${escapeHtml(stateLabel)}</em></summary>
      <div class="email-forensic-body email-geotrace">
        ...
        <div class="email-geotrace-map">${map}</div>
        ...
        <p>Approximate infrastructure positions only. NAT, relays, VPNs, cloud infrastructure and compromised systems can separate observed infrastructure from a human operator.</p>
      </div>
    </details>`;
  }
  ...
  function renderCampaignMemoryDetails(campaignMemory) {
    ...
    const entities = common.length
      ? `<div class="email-campaign-entities" aria-label="Matched campaign entities">${common.slice(0, 8).map((item) => `<article><div><strong>${escapeHtml(titleCase(item.entity_type || 'entity'))}</strong><code>${escapeHtml(item.entity_value || 'Unavailable')}</code></div><span>weight ${escapeHtml(String(item.weight ?? '—'))}</span><em>${escapeHtml(titleCase(item.trust_level || 'observed'))}</em></article>`).join('')}</div>`
    ...
    const relatedLinks = related.length
      ? `<div class="email-related-scans"><strong>Related investigations</strong>${related.slice(0, 8).map((item) => `<a href="/phishing?scan_id=${encodeURIComponent(item.scan_id || '')}"><span>${escapeHtml(String(item.scan_id || '').slice(0, 12))}${item.scan_id ? '…' : 'Unknown scan'}</span><em>score ${escapeHtml(String(item.score ?? '—'))} →</em></a>`).join('')}</div>`
      : '';
    return `<details class="email-forensic-panel" data-usp-panel="campaign"${correlated ? ' open' : ''}>
      <summary><span class="email-forensic-index">03</span><span><strong>MailGraph Campaign Memory™</strong><small>Deterministic, weighted and privacy-minimized correlation explains why investigations connect.</small></span><em>${correlated ? 'Correlated' : campaignMemory.state === 'UNAVAILABLE' ? 'Unavailable' : 'Conservative no-match'}</em></summary>
      <div class="email-forensic-body">
        ${summary}
        ${entities}
        ${relatedLinks}
  ...
  function renderPassportDetails(passport, scanId) {
    ...
    const exportActions = exportBase
      ? `<div class="email-export-actions" aria-label="Evidence export actions">
          <a class="btn btn-secondary" href="${exportBase}/json">Evidence JSON</a>
          <a class="btn btn-secondary" href="${exportBase}/stix">STIX 2.1</a>
          <a class="btn btn-secondary" href="${exportBase}/csv">IOC CSV</a>
          <a class="btn btn-secondary" href="${exportBase}/iocs">IOC JSON</a>
        </div>`
      : '';
    if (!passport?.passport_id) {
      return `<details class="email-forensic-panel" data-usp-panel="passport"><summary><span class="email-forensic-index">04</span><span><strong>Evidence Passport™</strong><small>Cryptographic investigation provenance.</small></span><em>Unavailable</em></summary><div class="email-forensic-body"><p class="email-detail-empty">This investigation does not contain a signed Evidence Passport. Review recorded limitations before relying on exported evidence.</p>${exportActions}</div></details>`;
    }
    return `<details class="email-forensic-panel" data-usp-panel="passport" open>
      <summary><span class="email-forensic-index">04</span><span><strong>Evidence Passport™</strong><small>SHA-256 integrity binding plus Ed25519 authenticity and trusted-issuer verification.</small></span><em>${escapeHtml(passport.signature_algorithm || 'Ed25519')}</em></summary>
      <div class="email-forensic-body">
        <div class="email-passport-card">
          <div><span>Passport ID</span><code>${escapeHtml(passport.passport_id)}</code></div>
          <div><span>Signing algorithm</span><strong>${escapeHtml(passport.signature_algorithm || 'Ed25519')}</strong></div>
          <div><span>Signing key ID</span><code>${escapeHtml(passport.key_id || 'Unavailable')}</code></div>
          <div><span>Key source</span><strong>${escapeHtml(titleCase(passport.key_source || 'not recorded'))}</strong></div>
          <div><span>Evidence SHA-256</span><code>${escapeHtml(passport.evidence_sha256 || 'Unavailable')}</code></div>
          <div><span>Manifest SHA-256</span><code>${escapeHtml(passport.manifest_sha256 || 'Unavailable')}</code></div>
        </div>
        ...
        ${exportActions}
        ${scanId ? `<div class="email-passport-verify"><a class="btn btn-primary" href="/verify-evidence?scan_id=${encodeURIComponent(scanId)}">Verify Passport</a></div>` : ''}
      </div>
    </details>`;
  }

  function renderForensicDetails({ evidence, infrastructure, infrastructureSummary, campaignMemory, passport, scanId }) {
    return `<section class="email-forensic-details" aria-labelledby="forensicDetailsTitle">
      <div class="email-forensic-heading"><div><p class="email-result-kicker">Forensic evidence details</p><h3 id="forensicDetailsTitle">See the proof behind the four capabilities</h3></div><p>Open only the sections you need. The complete PDF remains the analyst-ready report.</p></div>
  ...
  function renderResult(payload) {
    ...
    target.innerHTML = `
      ...
      ${renderForensicDetails({ evidence, infrastructure, infrastructureSummary, campaignMemory, passport, scanId: payload.scan_id })}
      <section class="email-essential-signals" aria-labelledby="essentialSignalsTitle">
        ...
        <p>The browser summarizes the four evidence-trust capabilities above. The complete PDF preserves the analyst-ready report, full technical provenance, limitations, and response details.</p>
      </section>`;
    ...
    all('[data-upgrade-evidence]', target).forEach((button) => button.addEventListener('click', (event) => {
