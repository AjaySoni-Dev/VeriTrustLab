(function emailInvestigationPage(global) {
  ...
  function renderEvidenceIntegritySection(payload, evidence, passport) {
    const integrity = payload.evidence_integrity || evidence.evidence_integrity || null;
    const signed = Boolean(passport?.passport_id);
    const original = integrity?.original_passport || null;
    const metadata = signed ? passport : original;
    const status = signed ? 'SIGNED PACKAGE AVAILABLE' : integrity?.status === 'RECONSTRUCTED_UNSIGNED' ? 'RECONSTRUCTED — UNSIGNED' : 'NOT RECORDED';
    const explanation = signed
      ? 'The exact evidence package for this investigation includes an issued Evidence Passport. Use the verifier to check hashes, signature, Passport ID, key fingerprint, and current issuer trust.'
      : integrity?.status === 'RECONSTRUCTED_UNSIGNED'
        ? 'This historical report was reconstructed from persisted forensic records. Its original Passport metadata is preserved where available, but the exact originally hashed evidence is unavailable, so this reconstruction is not presented as that signed package.'
        : 'No durable Evidence Passport was originally recorded for this historical report. VeriTrust does not create a replacement Passport during read or export.';
    const action = signed
      ? `<a class="btn btn-secondary" href="/verify-evidence?scan_id=${encodeURIComponent(payload.scan_id || '')}">Verify evidence</a><button class="btn btn-secondary" type="button" data-export-evidence="signed">Export signed Evidence JSON</button>`
      : `<button class="btn btn-secondary" type="button" data-export-evidence="reconstructed">Export reconstructed JSON (unsigned)</button>`;
    const value = (label, entry) => `<div><dt>${escapeHtml(label)}</dt><dd>${escapeHtml(entry || 'Not recorded')}</dd></div>`;
    return `<section class="email-integrity-panel" aria-labelledby="emailIntegrityTitle" data-integrity-state="${signed ? 'signed' : 'unsigned'}">
      <div class="email-integrity-heading"><div><p class="email-result-kicker">Evidence integrity</p><h3 id="emailIntegrityTitle">Evidence Passport™</h3></div><strong>${escapeHtml(status)}</strong></div>
      <p>${escapeHtml(explanation)}</p>
      <dl class="email-integrity-meta">
        ${value('Passport ID', metadata?.passport_id)}
        ${value('Issued at', metadata?.issued_at)}
        ${value('Evidence SHA-256', metadata?.evidence_sha256)}
        ${value('Manifest SHA-256', metadata?.manifest_sha256)}
        ${value('Key fingerprint', metadata?.key_id)}
        ${value('Algorithm', metadata?.signature_algorithm)}
        ${value('Issuer state', signed ? 'Check current issuer trust in verifier' : 'Not checked for unsigned reconstruction')}
      </dl>
      <div class="email-integrity-actions">${action}</div>
      <small>Cryptographic integrity does not establish legal admissibility, sender/person identity, attacker identity, message safety, or correctness of the phishing verdict.</small>
    </section>`;
  }

  async function downloadEvidenceExport(scanId, signedExpected, button) {
    if (!scanId) throw new Error('The investigation scan ID is unavailable.');
    const original = button?.textContent || '';
    if (button) { button.disabled = true; button.textContent = 'Preparing export…'; }
    try {
      const response = await global.fetch(`/api/v2/phishing/export/${encodeURIComponent(scanId)}?format=json`, {
        method: 'GET', credentials: 'same-origin', cache: 'no-store', headers: { Accept: 'application/json' },
      });
      if (!response.ok) {
        const errorPayload = await response.json().catch(() => null);
        throw new Error(errorPayload?.error?.message || 'Evidence export could not be prepared.');
      }
      const blob = await response.blob();
      const text = await blob.text();
      let parsed;
      try { parsed = JSON.parse(text); } catch { throw new Error('Evidence export returned invalid JSON.'); }
      const isSigned = parsed?.export_integrity === 'signed_evidence_package' && Boolean(parsed?.evidence?.evidence_passport);
      if (signedExpected && !isSigned) throw new Error('The server did not return an exact signed Evidence Passport package; no signed export was downloaded.');
      const downloadBlob = new Blob([text], { type: 'application/json' });
      const url = URL.createObjectURL(downloadBlob);
      const anchor = document.createElement('a');
      anchor.href = url;
      anchor.download = isSigned ? `veritrust-${scanId}-evidence.json` : `veritrust-${scanId}-reconstructed-evidence.json`;
  ...
  function renderResult(payload) {
    ...
    target.innerHTML = `
      ...
      <section class="email-essential-signals" aria-labelledby="essentialSignalsTitle">
        ...
        <p>The browser view shows decision-critical information first. Authentication details, relay reconstruction, GeoTrace context, domain/IP intelligence, campaign entities, provenance, limitations, and complete response data remain available in the full report.</p>
      </section>
      ${renderEvidenceIntegritySection(payload, evidence, passport)}`;
    ...
    all('[data-export-evidence]', target).forEach((button) => button.addEventListener('click', async () => {
      try {
        await downloadEvidenceExport(payload.scan_id, button.dataset.exportEvidence === 'signed', button);
