(function emailInvestigationPage(global) {
  ...
  function essentialSignals({ deterministic, authentication, relationships, children, threatIntelligence }) {
    ...
    authentication.filter((item) => ['FAIL', 'SOFTFAIL', 'PERMERROR', 'TEMPERROR'].includes(String(item.result || '').toUpperCase())).slice(0, 2)
      .forEach((item) => {
        ...
        signals.push({ label: `${String(item.protocol || 'Sender check').replaceAll('_', ' ')}: ${friendlyStatus(item.result)}`, tone: result === 'FAIL' ? 'danger' : 'attention' });
      });
  ...
  function renderProgressiveEvidenceDetails(evidence, scanId) {
    ...
    const linkChildren = (Array.isArray(evidence.children) ? evidence.children : []).filter((item) => item.type === 'url');
    const linkAnalysis = completeness.link_analysis || {
      extracted: linkChildren.length,
      completed: linkChildren.filter((item) => item.state === 'completed').length,
      failed: linkChildren.filter((item) => item.state === 'failed').length,
      timed_out: linkChildren.filter((item) => item.state === 'timed_out').length,
      pending: linkChildren.filter((item) => !['completed', 'failed', 'timed_out'].includes(item.state)).length,
    };
    ...
    const linkAudit = linkChildren.length ? `<details class="email-link-audit">
      <summary><span>Link analysis coverage</span><strong>${escapeHtml(String(linkAnalysis.completed || 0))}/${escapeHtml(String(linkAnalysis.extracted ?? linkChildren.length))} analyzed</strong></summary>
      <p>Every extracted clickable HTTP(S) link is tracked individually. Embedded resource URLs such as image pixels are not counted as user-clickable links.</p>
      <div class="email-link-audit-list">${linkChildren.map((item, index) => {
        const meta = item.metadata || {};
        const destination = [meta.hostname, meta.path && meta.path !== '/' ? meta.path : ''].filter(Boolean).join('') || `Link ${index + 1}`;
        const score = Number.isFinite(Number(item.score)) ? `${Math.round(Number(item.score) * 100)}%` : 'No score';
        return `<article data-link-state="${escapeHtml(String(item.state || 'unknown').toLowerCase())}"><span>${index + 1}</span><div><strong>${escapeHtml(destination)}</strong><small>${escapeHtml(friendlyStatus(item.state || 'unknown'))} · ${escapeHtml(titleCase(item.verdict || 'unknown'))} · ${escapeHtml(score)}</small></div></article>`;
      }).join('')}</div>
    </details>` : '<p class="email-detail-empty">No clickable HTTP(S) link was extracted from this email.</p>';
  ...
  function renderGeoTraceDetails(infrastructure, infrastructureSummary) {
    ...
    const map = points.length
      ? `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Approximate independently geolocated mail infrastructure observations"><g class="email-geotrace-grid">${grid}</g>${nodes}</svg>`
    ...
    return `<section class="email-usp-tabpanel email-geotrace" id="uspPanelGeotrace" role="tabpanel" aria-labelledby="uspTabGeotrace" data-usp-panel="geotrace" hidden>
      ...
      <div class="email-section-heading"><h4>Approximate infrastructure map</h4><p>Markers are independently geolocated from the selected source-side relay IPs. No connecting route is drawn because Received headers do not prove a physical network path or a sender's location.</p></div><div class="email-geotrace-map">${map}</div>
      ...
      <div class="email-section-heading"><h4>Mail infrastructure observations</h4></div>
