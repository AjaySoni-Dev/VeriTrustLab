(function emailInvestigationPage(global) {
  ...
  const HELP = Object.freeze({
    authentication: 'Email authentication evaluates protocol-level authorization and domain-alignment signals such as SPF, DKIM, DMARC and ARC when the required evidence exists. It does not authenticate a person or prove the message is safe.',
    ...
    dkim: 'DKIM verifies a domain signature and whether the signed message parts still match what was signed. It does not prove the human sender or the message intent.',
    ...
    arc: 'ARC records authentication assessments asserted by forwarding mail systems and lets the chain be validated. A valid chain does not by itself make those systems trusted or prove the content is safe.',
    ...
    acquisitionStage: 'The acquisition stage tells you how the email evidence was obtained. Pasted text has the least transport context, a raw .eml adds message-carried structure and headers, and a trusted receiver adds facts observed directly at the SMTP boundary.',
    ...
    campaignMemory: 'Campaign Memory compares normalized forensic entities from this investigation with prior investigations in the same workspace. Hashed lookup keys are used, while normalized entity values can also be retained for explainability. A match is tenant-scoped correlation evidence, not attribution to a person, actor, organization, or globally verified campaign.',
    ...
    privacyMinimized: 'Campaign matching uses normalized entity values and hashed lookup keys rather than full message bodies as the matching primitive. Normalized values may still be stored so a match can be explained.',
    ...
    ioc: 'The observed-artifact export lists domains, URLs, IP addresses, and file hashes seen in the investigation. Presence in the export does not independently classify an artifact as malicious.',
  });
  ...
  function setBusy(busy) {
    ...
    button.textContent = busy ? 'Checking email...' : state.mode === 'eml' ? 'Check raw .eml' : 'Check email text';
  ...
  function setMode(mode, focusPanel = false) {
    ...
    if (auth) auth.textContent = rawEvidence ? 'Available (SPF limited)' : 'Needs raw .eml';
    if (identity) identity.textContent = rawEvidence ? 'Available' : 'Limited with pasted text';
    if (infra) infra.textContent = rawEvidence ? 'Available when recorded' : 'Needs raw .eml';
    if (attachments) attachments.textContent = rawEvidence ? 'Available' : 'Needs raw .eml';
    setError('');
    setBusy(false);
    if (focusPanel) {
  ...
  function validateFile(file) {
    ...
    if (!extensionOk || !typeOk) throw new Error('Choose a raw .eml file with a supported email content type.');
    return file;
  }
  ...
  async function loadSavedInvestigation(scanId) {
    ...
    setStatus('Loading the saved investigation...');
    let lastError = null;
    for (let attempt = 0; attempt < 4; attempt += 1) {
      try {
        ...
        if (!payload.evidence || !payload.gateway_decision) throw new Error('The saved scan does not contain the required email evidence and decision.');
    ...
    renderFailure(lastError || new Error('The saved investigation could not be loaded.'));
    setStatus('The saved investigation could not be loaded.');
  }
  ...
  function evidenceStageName(mode) {
    return ({ plain_text: 'Pasted text', raw_eml: 'Raw .eml', trusted_receiver_event: 'Trusted receiver' })[mode] || 'Pasted text';
  }
  ...
  function renderProgressiveEvidenceDetails(evidence, scanId) {
    ...
    const stages = [
      ...
      ['Raw .eml', 'Message-carried headers, identity, MIME, attachment metadata/hashes and recorded relays'],
      ['Trusted receiver', 'Direct SMTP facts, SPF and configured trust boundary'],
    ];
    ...
    }).join('') + `<div class="email-evidence-dimension"><span>Attachment metadata / Links</span><strong>${escapeHtml(friendlyStatus(dimensions.attachments || 'UNKNOWN'))} / ${escapeHtml(friendlyStatus(dimensions.links || 'UNKNOWN'))}</strong></div>`;
    ...
    const nextMarkup = nextActions.length ? nextActions.map((item) => {
      ...
      const action = item.action === 'UPLOAD_ORIGINAL_EML'
        ? `<a class="btn btn-secondary" href="/phishing?mode=eml&parent_scan_id=${encodeURIComponent(scanId || '')}">Add raw .eml</a>`
    ...
    const parent = lineage.parent_scan_id
      ? `<div class="email-lineage"><span>Investigation lineage</span><strong>Linked to a prior investigation as an evidence upgrade</strong><a href="${resultPath(lineage.parent_scan_id, 'progressive')}">Open previous scan ${escapeHtml(String(lineage.parent_scan_id).slice(0, 8))}… →</a></div>`
      : '';
  ...
  function renderGeoTraceDetails(infrastructure, infrastructureSummary) {
    ...
    const geoHops = publicHops.filter((hop) => {
      const latitudePresent = hop.latitude !== null && hop.latitude !== undefined && hop.latitude !== '';
      const longitudePresent = hop.longitude !== null && hop.longitude !== undefined && hop.longitude !== '';
      const latitude = Number(hop.latitude);
      const longitude = Number(hop.longitude);
      return latitudePresent && longitudePresent
    ...
    // GeoTrace markers are evidence-backed coordinates. We intentionally do not
    // draw straight connector lines: a Received-header sequence is not an
    // observed network route, and a naive polyline is geometrically wrong across
    // the antimeridian. The ordered hop list below preserves sequence truthfully.
    ...
    const map = points.length
      ? `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Approximate mail infrastructure map with numbered relay hops"><g class="email-geotrace-grid">${grid}</g>${nodes}</svg>`
    ...
    const publicCount = Number(infrastructureSummary.public_hop_count ?? publicHops.length);
    const coordinateCount = Number(infrastructureSummary.coordinate_hop_count ?? geoHops.length);
    const mapState = infrastructureSummary.map_state || (coordinateCount ? (coordinateCount === publicCount ? 'COMPLETED' : 'PARTIAL') : 'UNAVAILABLE');
    const mapCoverage = mapState === 'COMPLETED'
      ? `Coordinates available for all ${coordinateCount} public infrastructure hop${coordinateCount === 1 ? '' : 's'}.`
      : (mapState === 'PARTIAL'
        ? `Coordinates available for ${coordinateCount} of ${publicCount} public infrastructure hops; unmapped hops remain in the provenance list.`
        : 'No public relay returned usable latitude/longitude. Relay provenance remains available below.');
    ...
    return `<section class="email-usp-tabpanel email-geotrace" id="uspPanelGeotrace" role="tabpanel" aria-labelledby="uspTabGeotrace" data-usp-panel="geotrace" hidden>
      <header class="email-usp-panel-header">
        <div class="email-capability-heading"><h3>Trust-Boundary GeoTrace™</h3><p class="email-capability-subtitle">Infrastructure Provenance &amp; Trust-Boundary Status</p></div>
      ...
      <div class="email-section-heading"><h4>Approximate infrastructure map</h4><p>${escapeHtml(mapCoverage)} Coordinates describe mail infrastructure only; they do not identify a sender or attacker.</p></div><div class="email-geotrace-map" data-map-state="${escapeHtml(String(mapState).toLowerCase())}">${map}</div>
  ...
  function renderCampaignMemoryDetails(campaignMemory) {
    ...
    const summary = correlated
      ? `<div class="email-campaign-summary"><strong>${escapeHtml(campaignMemory.campaign_id || 'Correlation group')}</strong><span>${escapeHtml(String(campaignMemory.related_scan_count || related.length))} prior investigation(s) · strongest weighted match ${escapeHtml(String(campaignMemory.strongest_match_score ?? '—'))}</span></div>`
      : `<div class="email-campaign-summary"><strong>${campaignMemory.state === 'UNAVAILABLE' ? 'Campaign correlation unavailable' : (campaignMemory.state === 'PARTIAL' ? 'Campaign search incomplete' : 'No strong prior correlation match')}</strong><span>${campaignMemory.state === 'PARTIAL' ? 'The bounded historical lookup was truncated; no definitive no-match claim is made.' : (weakCount ? `${escapeHtml(String(weakCount))} weak candidate${weakCount === 1 ? '' : 's'} rejected` : 'No sufficiently strong prior overlap')}</span></div>`;
    ...
    const relatedLinks = related.length
      ...
      : '';
    const stateLabel = correlated ? 'Correlated' : campaignMemory.state === 'UNAVAILABLE' ? 'Unavailable' : campaignMemory.state === 'PARTIAL' ? 'Partial lookup' : 'Conservative no-match';
    ...
    return `<section class="email-usp-tabpanel" id="uspPanelCampaign" role="tabpanel" aria-labelledby="uspTabCampaign" data-usp-panel="campaign" hidden>
      <header class="email-usp-panel-header">
        <div class="email-capability-heading"><h3>MailGraph Campaign Memory™</h3><p class="email-capability-subtitle">Prior-Investigation Entity Correlation</p></div>
  ...
  function renderPassportDetails(passport, scanId) {
    ...
    const exportActions = exportBase
      ? `<div class="email-export-actions" aria-label="Evidence export actions">
          ...
          <details class="email-more-exports"><summary>More exports</summary><div><a href="${exportBase}/stix">STIX 2.1</a><a href="${exportBase}/csv">Observed-artifact CSV</a><a href="${exportBase}/iocs">Observed-artifact JSON</a></div></details>
        </div>`
      : '';
    ...
    return `<section class="email-usp-tabpanel" id="uspPanelPassport" role="tabpanel" aria-labelledby="uspTabPassport" data-usp-panel="passport" hidden>
      <header class="email-usp-panel-header">
        <div class="email-capability-heading"><h3>Evidence Passport™</h3><p class="email-capability-subtitle">Signed Package Integrity &amp; Issuer Checks</p></div>
  ...
  function init() {
    ...
    async function recoverInterruptedInvestigation(originalError) {
      ...
      if (candidateScanId && /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(candidateScanId)) {
        setStatus('Finalizing investigation evidence from the server...');
        analysisProgress.update({ stage: 'decision', state: 'running', message: 'Retrieving the final investigation report from the server...' });
    ...
    if (/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(requestedParent)) {
      ...
      setStatus('Evidence upgrade linked to the previous investigation. Add a raw .eml export to create the next acquisition stage.');
    }
