(function analysisProgressModule(global) {
  ...
  function create() {
    ...
    function finish(error = null, { pending = false, title, message } = {}) {
      ...
      if (current) current.textContent = error ? friendlyErrorMessage(error) : pending ? 'The server confirmed that this scan is already running. Open its saved status to follow it.' : 'The investigation is complete. Review the concise result or open the generated PDF report.';
