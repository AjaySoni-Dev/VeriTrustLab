(function emailReportPdf(global) {
  ...
  function renderForensicSections(document, evidence, decision) {
    ...
    if (passport.passport_id) {
      ...
      document.finding('PDF signature boundary', 'This PDF is a human-readable rendering. Cryptographic integrity is checked against the exported Evidence JSON / Evidence Passport; the PDF bytes themselves are not covered by the Passport unless separately signed.', 'PDF bytes are not Passport-signed');
    } else {
      document.finding('Evidence Passport unavailable', 'This report does not contain an exact signed Evidence Passport package. Treat the report as an unsigned reconstruction.', 'Integrity limitation');
      document.finding('PDF signature boundary', 'This PDF is a human-readable rendering and is not itself covered by an Evidence Passport signature unless separately signed.', 'Unsigned PDF rendering');
    }
