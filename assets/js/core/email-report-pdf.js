(function emailReportPdf(global) {
  ...
  function renderGeoTraceFigure(document, infrastructure) {
    ...
    document.text('Independent infrastructure positions - no inferred physical route', PAGE_WIDTH - MARGIN - 14, top + 11, 6.9, false, MUTED_COLOR, 'right');
  ...
  function renderForensicSections(document, evidence, decision) {
    ...
    const limitations = [...new Set(list(evidence.limitations))];
    ...
    document.runningSection = 'ESSENTIAL FORENSIC FINDINGS';
    document.newPage();
    document.sectionTitle('Evidence and acquisition', 'Essential findings');
    ...
    document.finding(`Current evidence stage: ${acquisitionStage}`, completeness.wording || 'Evidence coverage and threat risk are separate concepts.', 'Progressive Evidence Escalation™');
    const nextAction = list(completeness.next_actions)[0];
    if (nextAction) {
      const action = nextAction.action || 'EVIDENCE_UPGRADE';
      const message = action === 'UPLOAD_ORIGINAL_EML'
        ? 'Upload the original .eml to add sender authentication, MIME, attachment and relay evidence.'
        : action === 'USE_TRUSTED_RECEIVER'
          ? 'Use the trusted SMTP receiver to add directly observed transport facts and SPF.'
          : 'Collect the requested stronger evidence and rerun the investigation.';
      document.finding('Recommended evidence upgrade', message, titleCase(action));
    }

    document.sectionTitle('Decision signals');
    const signalCodes = [...new Set([...findings.map((item) => item.code), ...list(decision.reason_codes)].filter(Boolean))];
    if (!signalCodes.length) document.finding('No high-priority deterministic signal recorded', 'The result is based on the available model, sender and policy evidence. Absence of a signal does not prove safety.');
    signalCodes.slice(0, 6).forEach((code) => document.finding(FINDING_LABELS[code] || titleCase(code), `Recorded technical code: ${code}`, 'Decision evidence'));
    if (signalCodes.length > 6) document.finding('Additional decision evidence', `${signalCodes.length - 6} additional technical signal(s) remain available in the machine-readable evidence export.`, 'Condensed report');

    document.sectionTitle('Sender verification and identity');
    if (!authentication.length) document.finding('Sender authentication unavailable', 'The submitted evidence did not contain enough trusted information for a recorded authentication result.');
    authentication.slice(0, 4).forEach((item) => {
      const result = titleCase(item.result || item.status || 'Unknown');
      const domain = item.domain ? ` for ${item.domain}` : '';
      document.finding(String(item.protocol || 'Sender check').replaceAll('_', ' '), `${result}${domain}.${item.failure_reason ? ` Limitation: ${titleCase(item.failure_reason)}.` : ''}`, 'Authentication');
    });
    const importantRelationships = relationships.filter((item) => {
      const code = String(item.reason_code || '');
      return code && !/(?:ALIGNED|SENDING_INFRASTRUCTURE_OBSERVED)$/u.test(code);
    });
    importantRelationships.slice(0, 5).forEach((item, index) => {
      const target = item.target_value || item.target || item.domain || 'No target value recorded';
      document.finding(FINDING_LABELS[item.reason_code] || titleCase(item.edge_type || `Identity relationship ${index + 1}`), `${titleCase(item.source_type || 'sender')} to ${titleCase(item.target_type || 'related identity')}: ${target}.`, item.reason_code || 'Identity evidence');
    });
    if (!importantRelationships.length && relationships.length) document.finding('No material sender-identity mismatch highlighted', 'Recorded identity relationships did not produce a high-priority mismatch for this concise report.', 'Identity evidence');

    document.sectionTitle('Links and attachments');
    const links = children.filter((item) => item.type === 'url');
    const attachments = children.filter((item) => item.type === 'attachment');
    const completedLinks = links.filter((item) => item.state === 'completed').length;
    const failedLinks = links.filter((item) => ['failed', 'timed_out'].includes(item.state)).length;
    document.finding('Link analysis coverage', links.length ? `${completedLinks}/${links.length} extracted clickable HTTP(S) links completed specialist analysis${failedLinks ? `; ${failedLinks} failed or timed out` : ''}.` : 'No clickable HTTP(S) link was extracted.', 'Coverage');
    const riskyLinks = links.filter((item) => Number(item.score) >= 0.45 || ['malicious', 'suspicious'].includes(String(item.verdict || '').toLowerCase()) || !['completed'].includes(item.state));
    riskyLinks.slice(0, 6).forEach((item, index) => {
      const metadata = item.metadata || {};
      const destination = [metadata.hostname, metadata.path && metadata.path !== '/' ? metadata.path : ''].filter(Boolean).join('') || `Link ${index + 1}`;
      const score = Number.isFinite(Number(item.score)) ? `${Math.round(Number(item.score) * 100)}%` : 'No score';
      document.finding(destination, `${titleCase(item.state || 'unknown')} | ${titleCase(item.verdict || 'unknown')} | ${score}.`, 'Link specialist');
    });
    if (links.length && !riskyLinks.length) document.finding('No elevated link result', `All ${links.length} extracted clickable link(s) completed without an elevated link-specialist result.`, 'Link specialist');
    attachments.slice(0, 4).forEach((item, index) => {
      const metadata = item.metadata || {};
      const forensic = metadata.forensic_metadata || {};
      const flags = list(forensic.flags).map((flag) => flag.code).filter(Boolean);
      document.finding(metadata.original_filename_untrusted || `Attachment ${index + 1}`, `${titleCase(item.state || 'METADATA_ONLY')}${flags.length ? ` | metadata flags: ${flags.join(', ')}` : ''}. Attachment content was not executed.`, 'Attachment metadata');
    });
    if (attachments.length > 4) document.finding('Additional attachments', `${attachments.length - 4} additional attachment record(s) remain available in the machine-readable evidence export.`, 'Condensed report');

    document.sectionTitle('Trust-Boundary GeoTrace');
    document.finding('Origin reliability', infrastructureSummary.wording || 'Infrastructure observations are approximate and do not identify a person.', infrastructureSummary.state || 'Infrastructure context');
    renderGeoTraceFigure(document, infrastructure);
    const uniqueInfrastructure = [];
    const seenInfrastructure = new Set();
    infrastructure.filter((item) => item.ip_classification === 'public').forEach((item) => {
      const key = `${item.ip_address || ''}|${item.host || ''}`;
      if (!seenInfrastructure.has(key)) { seenInfrastructure.add(key); uniqueInfrastructure.push(item); }
    });
    if (!uniqueInfrastructure.length) document.finding('No public source-side relay available', 'No eligible public source-side relay IP was extracted. Receiver-side addresses are not substituted as sender-side infrastructure.');
    uniqueInfrastructure.slice(0, 5).forEach((item, index) => {
      const location = [item.city, item.region, item.country].filter(Boolean).join(', ');
      const detail = [item.host, item.ip_address, item.asn_org, location, item.trust_level ? `Trust: ${item.trust_level}` : ''].filter(Boolean).join(' | ');
      document.finding(`Infrastructure observation ${Number(item.hop_index ?? index) + 1}`, detail || 'No network detail recorded', item.trust_level === 'trusted_receiver' ? 'Trusted receiver observation' : 'Observed relay - origin unverified');
    });

    document.sectionTitle('Threat intelligence and Campaign Memory');
    const domains = list(threatIntelligence.domain_intelligence)
      .sort((a, b) => (Number(a.age_days) || Number.MAX_SAFE_INTEGER) - (Number(b.age_days) || Number.MAX_SAFE_INTEGER));
    const noteworthyDomains = domains.filter((item) => Number.isFinite(Number(item.age_days)) && Number(item.age_days) <= 90).slice(0, 4);
    noteworthyDomains.forEach((item) => document.finding(item.domain || 'Domain', `Registration age: ${item.age_days} days${item.registrar ? ` | Registrar: ${item.registrar}` : ''}.`, 'RDAP context'));
    const noteworthyIps = list(threatIntelligence.ip_reputation).filter((item) => Number(item.abuse_confidence_score) > 0 || item.is_tor).slice(0, 4);
    noteworthyIps.forEach((item) => document.finding(item.ip_address || 'IP address', `${Number.isFinite(Number(item.abuse_confidence_score)) ? `Abuse confidence: ${item.abuse_confidence_score}%` : 'Reputation signal recorded'}${item.is_tor ? ' | Tor exit node' : ''}.`, 'Third-party reputation'));
    if (!noteworthyDomains.length && !noteworthyIps.length) document.finding('No elevated external-intelligence item highlighted', 'Available registration and reputation context did not produce a concise-report highlight, or the provider was unavailable.', 'Context only');
    if (campaignMemory.state === 'CORRELATED') {
      document.finding(`Campaign ${campaignMemory.campaign_id || 'detected'}`, `${campaignMemory.related_scan_count || 0} prior investigation(s) meet the conservative deterministic correlation threshold.`, 'MailGraph Campaign Memory™');
      list(campaignMemory.common_entities).slice(0, 4).forEach((item) => document.finding(titleCase(item.entity_type || 'entity'), `${displayValue(item.entity_value)} | weight ${displayValue(item.weight, 'not recorded')}.`, 'Repeated forensic entity'));
    } else {
      document.finding('No strong prior campaign match', campaignMemory.scoring_notice || 'Weak single-signal overlap is intentionally insufficient to create a campaign.', 'MailGraph Campaign Memory™');
    }

    document.sectionTitle('AI assessment and evidence provenance');
    const model = models[0];
    if (model) {
      const likelihood = Number.isFinite(Number(model.p_phish)) ? `${Math.round(Number(model.p_phish) * 100)}% phishing likelihood` : 'No likelihood recorded';
      document.finding('Content model', `${titleCase(model.state || model.verdict || model.status || 'Unknown')} - ${likelihood}.`, 'Supporting signal, not the sole verdict');
    } else document.finding('AI model evidence unavailable', 'The absence of an AI result was not converted into a safe result.');
    document.finding('Pipeline provenance', `Schema ${displayValue(manifest.schema_version, 'not recorded')} | Pipeline ${displayValue(manifest.pipeline_version, 'not recorded')} | Parser ${displayValue(manifest.parser_version, 'not recorded')}.`, 'Reproducibility');
    if (passport.passport_id) {
      document.finding('Evidence Passport', `Passport ${shortenId(passport.passport_id)} | ${passport.signature_algorithm || 'Ed25519'} | key ${shortenId(passport.key_id || 'not recorded')}. Evidence hash ${shortenId(passport.evidence_sha256 || 'not recorded')}.`, 'Cryptographic integrity');
    } else document.finding('Evidence Passport unavailable', 'This report does not contain a signed Evidence Passport.', 'Integrity limitation');
    ...
    limitations.slice(0, 5).forEach((item) => document.finding(LIMITATION_LABELS[item] || titleCase(item), `Recorded technical code: ${item}`, 'Evidence gap'));
    if (limitations.length > 5) document.finding('Additional limitations', `${limitations.length - 5} additional limitation(s) are preserved in the machine-readable evidence export.`, 'Condensed report');
    document.finding('Report scope', 'This concise PDF contains decision-relevant evidence only. Full JSON, STIX and IOC exports preserve the machine-readable investigation details.', 'Full evidence remains exportable');
    document.finding('Human review boundary', 'This report supports investigation and triage. It is not a certificate that the email is safe or malicious.', 'Review boundary');
  }
  ...
  function buildEmailReportPdf(payload, options = {}) {
    ...
    document.keyValue('Important limitations', limitations.length ? `${limitations.slice(0, 4).map((item) => LIMITATION_LABELS[item] || titleCase(item)).join(' ')}${limitations.length > 4 ? ` (+${limitations.length - 4} more in evidence export)` : ''}` : 'No additional limitation was recorded for the checks that ran.');
    ...
    document.runningSection = 'ESSENTIAL FORENSIC REPORT';
  ...
  async function openEmailReportPdf(payload) {
    ...
    try {
      ...
      viewer.document.body.textContent = 'Preparing concise VeriTrust Lab forensic report…';
