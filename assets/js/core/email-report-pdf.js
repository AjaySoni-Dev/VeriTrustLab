(function emailReportPdf(global) {
  ...
  const GLOSSARY = Object.freeze([
    ['.eml file', 'A raw RFC 822 message export that can preserve message-carried headers, body content, recorded relay headers, and attachment data. Its authenticity is not established merely by being an .eml file.'],
    ...
    ['ARC', 'Authenticated Received Chain records authentication assessments asserted by forwarding mail systems and supports chain validation; it does not prove those systems are trustworthy or that the message is safe.'],
    ...
    ['URL or link intelligence', 'Analysis of URL strings found in the email without requiring the user to open the destination. The current path does not fetch the webpage.'],
    ...
    ['Evidence Passport', 'A signed package that binds investigation evidence and manifest hashes to an Ed25519 key. Verification checks package consistency and configured issuer trust; it does not establish content truth, identity, or legal admissibility.'],
    ['Campaign Memory', 'Deterministic, tenant-scoped cross-investigation correlation of repeated evidence entities such as attachment hashes, URL domains, and trusted-receiver SMTP IPs. A match is not actor attribution or a globally verified campaign.'],
  ...
  class PdfDocument {
    ...
    newPage() {
      ...
      this.text('EMAIL EVIDENCE REPORT', this.logo ? 172 : 139, 26, 7.5, true, '0.20 0.34 0.39');
  ...
  function renderForensicSections(document, evidence, decision) {
    ...
    document.lines('The sections below translate the returned evidence into reviewer-friendly language. Returned machine values are also included in the response-data appendix.', { size: 9.8, lineHeight: 14.5, color: MUTED_COLOR });
    ...
    const acquisitionStage = ({ plain_text: 'Pasted text', raw_eml: 'Raw .eml', trusted_receiver_event: 'Trusted SMTP receiver' })[evidence.input_mode] || titleCase(evidence.input_mode || 'Pasted text');
    ...
    if (nextActions.length) {
      nextActions.slice(0, 3).forEach((item, index) => {
        ...
        const message = item?.wording || item?.message || item?.reason || (action === 'UPLOAD_ORIGINAL_EML'
          ? 'Upload a raw .eml export to add message-carried headers, identity relationships, MIME structure, attachment metadata/hashes and recorded relay claims.'
    ...
    document.finding('Safety boundary', 'Evidence completeness describes what VeriTrust Lab could examine. Missing evidence is displayed as missing; a low-risk or benign classification does not mean those unavailable checks passed.', 'Coverage is not threat risk');
    ...
    if (!authentication.length) document.finding('No sender-verification result', 'The submitted evidence did not contain enough usable information for a recorded authentication result.');
    ...
    if (campaignMemory.state === 'CORRELATED') {
      document.finding(`Correlation ${campaignMemory.campaign_id || 'recorded'}`, `${campaignMemory.related_scan_count || 0} prior investigation(s) share forensic entities that meet the deterministic correlation threshold. Strongest weighted match: ${campaignMemory.strongest_match_score || 0}.`, 'Cross-investigation correlation');
    ...
    } else if (campaignMemory.state === 'PARTIAL') {
      document.finding('Campaign search incomplete', campaignMemory.scoring_notice || 'The bounded historical lookup was truncated, so no definitive no-match claim is made.', 'Partial deterministic correlation');
    } else if (campaignMemory.state === 'UNAVAILABLE') {
      document.finding('Campaign correlation unavailable', campaignMemory.scoring_notice || 'Prior-investigation correlation could not be completed.', 'Correlation unavailable');
    } else {
      document.finding('No strong prior correlation match', campaignMemory.scoring_notice || 'Weak overlap such as ASN alone is intentionally insufficient to create a correlation match.', 'Deterministic correlation');
    }
    ...
    if (passport.passport_id) {
      ...
      document.finding('Verification boundary', passport.statement || 'Full-package verification checks signature, signed hashes, key identity, and configured issuer trust; it does not establish content truth, identity, safety, attribution, or legal admissibility.', 'Not a legal certificate');
    } else {
    ...
    if (!limitations.length) document.finding('No additional limitation code recorded', 'No additional limitation code was recorded. Review evidence completeness and individual check states separately.');
    ...
    document.finding('Human review for consequential action', 'This report supports investigation and triage. Use human review and independent verification before consequential action; it is not a certificate that the email is safe or malicious.', 'Review boundary');
  }
  ...
  function buildEmailReportPdf(payload, options = {}) {
    ...
    document.keyValue('Evidence coverage', evidence.evidence_completeness?.wording || (decision.degraded ? 'Some checks were unavailable. Review the evidence gaps before taking action.' : 'No degraded-decision flag was recorded; review dimension-level evidence completeness separately.'));
    const limitations = list(evidence.limitations);
    ...
    document.runningSection = 'RESPONSE DATA - CONTINUED';
    document.newPage();
    document.sectionTitle('Response data', 'Technical appendix');
  ...
  async function openEmailReportPdf(payload) {
    ...
    try {
      ...
      viewer.document.body.textContent = 'Preparing VeriTrust Lab investigation report…';
