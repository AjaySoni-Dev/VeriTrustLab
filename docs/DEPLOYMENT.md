...
Use the version-matched `assets/downloads/VeriTrust-Lab-Persistent-CLI.zip` for the two-laptop Windows demonstration. The package bundles the receiver/sender scripts and matching `mail-gateway/` runtime so the demo does not download source from GitHub.
