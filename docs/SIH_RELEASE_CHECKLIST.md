...
5. Perform one real EML investigation and verify that threat entities and one update-protected Passport row with exact new-issuance evidence are written.
...
- When rotating a key, keep each still-trusted retired fingerprint in `VERITRUST_EVIDENCE_TRUSTED_KEY_IDS` for packages that must remain verifiable.
