...
12. **Evidence Passport** — the finalized normalized evidence is validated as JSON, canonicalized, SHA-256 bound to its manifest, signed once with Ed25519, self-verified, and durably persisted together with the exact signed evidence. Signature validity and issuer trust remain separate.
13. **Persistence/reporting** — results, exact new-issuance evidence, cases, campaign entities, Passport envelopes and audit identifiers use the Supabase contract. Historical reads hydrate the original Passport and never invoke the signer.
...
- `email_evidence_passports` stores update-protected Passport envelope/signature/hash metadata and, for new issuances after the hardening migration, the exact normalized `evidence_payload`. It is not WORM storage: UPDATE is blocked, while deletion follows the parent investigation retention/deletion lifecycle.

Both tables reference the existing `organizations`, `gateway_scans`, and `gateway_artifacts` records. `20260914_evidence_passport_hardening.sql` adds same-tenant/same-scan composite Passport foreign keys after historical-data preflight and preserves server-only RLS. For new signed investigations, durable exact-evidence persistence is a completion invariant: the service fails rather than returning a Passport it could not durably record.
