...
| URL intelligence | Every distinct extracted clickable HTTP(S) link is tracked and analyzed within the bounded email limit (50); image/resource `src` URLs do not consume the clickable-link budget. |
| Trust-Boundary GeoTrace | Source-side `Received:` hop extraction, IP classification, provider-derived ASN/approximate geo context, and explicit trusted-receiver vs observed-relay semantics. The UI plots independent observations and does not draw an inferred physical route. |
...
| Cases and reports | Persisted evidence, review/case workflows, concise decision-relevant PDF reports, identifiers, provenance, and machine-readable JSON/STIX/IOC exports. |
...
Email link analysis defaults to four concurrent link checks and can be bounded from 1–8 with `VERITRUST_LINK_ANALYSIS_CONCURRENCY`. The extraction cap remains 50 distinct clickable HTTP(S) URLs per email to preserve explicit resource limits.
