...
| `verify-evidence.html` | Local-first Evidence Passport cryptographic verification with separate current issuer-trust evaluation and explicit server fallback. |
...
| Evidence Passport | Durable exact-evidence provenance for new investigations: SHA-256 + Ed25519, strict v1 verification, rotation-aware issuer trust, historical no-resign behavior, and signed JSON export when the original package exists. |
