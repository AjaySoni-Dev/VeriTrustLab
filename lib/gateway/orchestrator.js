...
function assertSubmissionModulesEnabled(submission) {
  const media = Array.isArray(submission?.content?.media) ? submission.content.media : [];
  if (media.length && !isModuleEnabled('deepfake')) {
    throw new HttpError(422, 'Media analysis is disabled in this VeriTrust Lab deployment. Remove media artifacts or enable the deepfake module before submitting them.', { code: 'GATEWAY_MEDIA_MODULE_DISABLED' });
  }
}

async function orchestrateSubmission(input) {
  const { auth, submission, idempotencyKey, requestId, traceId } = input;
  assertSubmissionModulesEnabled(submission);
...
module.exports = {
  assertSubmissionModulesEnabled,
