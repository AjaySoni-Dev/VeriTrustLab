...
async function analyzeEml(req, onProgress = () => {}) {
  ...
  onProgress('validation', 'completed', 'Raw email bytes received and workspace access validated.');
...
async function receiverEml(req, onProgress = () => {}) {
  ...
  const raw = await readRawBuffer(req);
  onProgress('validation', 'completed', 'Trusted SMTP receiver facts and gateway-supplied message bytes validated.');
  return analyzeEmail({
    auth,
...
async function receiverEvent(req) {
  ...
  return analyzeEmail({ auth, mode: 'trusted_receiver_event', raw: object.buffer, receiver, parentScanId: parentScanId(req), integrationId: receiver.integration_id, idempotencyKey, requestId: ids.requestId, traceId: ids.traceId });
}
...
function validateEvidenceVerificationRequest(body) {
  if (!body || typeof body !== 'object' || Array.isArray(body)) {
    throw new HttpError(400, 'The verification request must be a JSON object.', { code: 'EVIDENCE_VERIFY_SCHEMA_INVALID' });
  }
  ...
  if (!body.passport || typeof body.passport !== 'object' || Array.isArray(body.passport)) {
    throw new HttpError(400, 'A VeriTrust Lab Evidence Passport is required.', { code: 'PASSPORT_REQUIRED' });
  }
  if (!body.evidence || typeof body.evidence !== 'object' || Array.isArray(body.evidence)) {
    throw new HttpError(400, 'The evidence object is required for full package integrity verification.', { code: 'EVIDENCE_PACKAGE_REQUIRED' });
  }
  return body;
}
...
async function verifyPassport(req) {
  validateJsonContentType(req);
  const body = validateEvidenceVerificationRequest(await parseJsonBody(req, 1024 * 1024));
...
module.exports = { analyzeEml, analyzeText, evidence, exportEvidence, readRawBuffer, receiverEml, receiverEvent, trustedReceiverMetadata, validateEvidenceVerificationRequest, verifyPassport };
