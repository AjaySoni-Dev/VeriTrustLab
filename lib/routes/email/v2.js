...
const { MAX_EVIDENCE_PACKAGE_BYTES, VERIFICATION_STATUS, evidenceTrustMetadata, trustedEvidenceKeyIds, verifyEvidencePackage } = require('../../email/evidence-passport');
...
async function evidence(req, scanId) {
  ...
  const gatewayDecision = storedDecision || (latestDecision ? {
    ...
    const integrity = report.evidence_passport ? {
    status: 'SIGNED_PACKAGE_AVAILABLE',
    signed_package_available: true,
    passport_id: report.evidence_passport.passport_id,
    key_id: report.evidence_passport.key_id,
  } : (report.evidence_integrity || { status: 'NOT_ORIGINALLY_RECORDED', signed_package_available: false });
  return { status: 200, body: { ok: true, scan_id: scanId, status: gateway.scan?.status || 'completed', gateway_decision: gatewayDecision, evidence_integrity: integrity, evidence: report }, replayed: false };
}
...
async function exportEvidence(req, scanId, format = 'json') {
  ...
  if (normalized === 'json') {
    const signed = Boolean(report.evidence_passport);
    return {
      contentType: 'application/json; charset=utf-8',
      filename: signed ? `veritrust-${scanId}-evidence.json` : `veritrust-${scanId}-reconstructed-evidence.json`,
      body: JSON.stringify({
        scan_id: scanId,
        export_integrity: signed ? 'signed_evidence_package' : 'reconstructed_unsigned',
        ...(signed ? {} : { limitation: report.evidence_integrity?.limitation || 'EVIDENCE_PASSPORT_NOT_ORIGINALLY_RECORDED' }),
        evidence: report,
      }, null, 2),
    };
  }
...
function evidenceTrust() {
  return { status: 200, body: { ok: true, ...evidenceTrustMetadata() } };
}

async function verifyPassport(req) {
  validateJsonContentType(req);
  await enforceRateLimit({ req, endpoint: 'evidence-passport-verify', limit: 100, identityType: 'ip' });
  const body = await parseJsonBody(req, MAX_EVIDENCE_PACKAGE_BYTES);
  ...
  const verification = verifyEvidencePackage(body, { trustedKeyIds: trustedEvidenceKeyIds() });
  const failed = [VERIFICATION_STATUS.VERIFICATION_FAILED, VERIFICATION_STATUS.UNTRUSTED_ISSUER].includes(verification.status);
  return { status: failed ? 422 : 200, body: { ok: verification.valid, verification } };
}

module.exports = { analyzeEml, analyzeText, evidence, evidenceTrust, exportEvidence, readRawBuffer, receiverEml, receiverEvent, trustedReceiverMetadata, verifyPassport };
