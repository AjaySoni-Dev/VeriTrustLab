...
async function createApiKey(context, options = {}) {
  ...
  const name = String(options.name || 'VeriTrust Lab API Key').trim().slice(0, 80) || 'VeriTrust Lab API Key';
...
async function recordApiUsage(auth, data = {}) {
  ...
  try {
    ...
    if (error instanceof SupabaseError) {
      console.error('VeriTrust Lab API usage logging failed', {
        status: error.status,
...
function externalApiError(res, error, fallback = 'Request failed.', request_id = requestId()) {
  ...
  if (safeStatus >= 500) {
    console.error('VeriTrust Lab external API error', {
      request_id,
      code,
      status: safeStatus,
      message: error.message,
