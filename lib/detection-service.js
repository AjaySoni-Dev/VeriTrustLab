...
function reportPayload(scanId, scanType, payload, createdAt, disclaimer) {
  return {
    ...
    report: {
      title: 'VeriTrust Lab Scan Report',
      disclaimer,
      exportable: true,
    },
  };
...
function linkReportPayload(scanId, payload, createdAt) {
  return {
    ...
    scores: payload.scores || [],
    report: {
      title: 'VeriTrust Lab Link Intelligence Report',
...
async function runLinkDetection({
  ...
  for (const key of modelOrder) {
    ...
    try {
      ...
      onProgress('model-swift', 'running', 'Requesting URL classification from VeriTrust Lab Swift.');
