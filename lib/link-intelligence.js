...
function validateLinkModel(modelKey) {
  ...
  if (LINK_MODELS[normalized].locked || LINK_MODELS[normalized].comingSoon) {
    throw new HttpError(400, 'VeriTrust Lab Sentinel is not available in this version.', { code: 'INVALID_MODEL' });
  }
  return normalized;
}
...
function analyzeUrlRules(url, context = '') {
  ...
  if (domain.length >= 38) {
    indicators.push(indicator('long_domain', 'Very long domain', 'The registered-domain portion crosses the configured long-domain heuristic threshold.', 'Medium'));
  }

  const hyphenCount = (hostname.match(/-/g) || []).length;
  if (hyphenCount >= 4) {
    indicators.push(indicator('too_many_hyphens', 'Many hyphens in domain', 'The hostname crosses the configured multi-hyphen heuristic threshold.', 'Medium'));
  }
  ...
  if (SUSPICIOUS_TLDS.has(tld)) {
    indicators.push(indicator('suspicious_tld', 'TLD heuristic match', 'The top-level domain is in the configured heuristic watchlist.', 'Medium'));
  }
  ...
  if (hasTerm(lower, INDIA_TERMS)) {
    indicators.push(indicator('india_scam_terms', 'India-relevant risk-term heuristic', 'The URL or context matches the configured banking, UPI, wallet, KYC, delivery, loan, or identity-term heuristic.', 'Medium'));
  }
  ...
  for (const key of parsed.searchParams.keys()) {
    if (REDIRECT_PARAMS.has(key.toLowerCase())) {
      indicators.push(indicator('redirect_query', 'Redirect-style query parameter', 'The query string includes a redirect-style parameter that can send users to another destination.', 'Medium'));
      break;
    }
  }
  ...
  if (brandHits.length && hasTerm(lower, ['official', 'support', 'secure', 'verify', 'kyc', 'refund', 'reward'])) {
    indicators.push(indicator('official_brand_like_terms', 'Brand-like wording with risk terms', 'Brand-like wording appears together with official, support, verification, KYC, refund, or reward terms.', 'High'));
  }
...
function labelFromScore(score, modelLabel, indicators) {
  ...
  if (normalized < 0.45) return 'Lower risk';
  return 'Unknown';
}

function summaryFor(label, indicators) {
  const count = Array.isArray(indicators) ? indicators.length : 0;
  if (label === 'Malicious') return 'The combined model/rule score crossed the configured malicious-risk threshold. Verify the destination through an official channel before use.';
  if (label === 'Phishing') return 'The combined model/rule evidence crossed the configured phishing-risk threshold.';
  if (label === 'Suspicious') return count
    ? 'The URL triggered multiple configured risk heuristics.'
    : 'The URL has limited suspicious signals. Manual verification is recommended.';
  if (label === 'Lower risk') return count
    ? 'Available URL-string checks found no high-risk indicators, though lower-level heuristic flags remain for review. This does not establish destination safety.'
    : 'Available URL-string checks found no strong risk indicators. This does not establish destination safety.';
