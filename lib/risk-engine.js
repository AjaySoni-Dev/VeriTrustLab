...
function buildDeepfakeEvidence(payload = {}) {
  ...
  if (model.fallback_used || model.fallback_from) {
    evidence.push(indicator(
      'model_fallback',
      'Backup model used',
      'The selected model was unavailable, so VeriTrust Lab used a configured backup model.',
      'Medium'
    ));
  }
...
function suspiciousDomains(entities) {
  ...
  return entities.domains.filter((domain) => {
    ...
    const riskyWord = /(?:login|verify|secure|account|update|wallet|bank|kyc)/i.test(domain);
    const structuralAnomaly = labels.length >= 5 || /[0-9]{3,}/.test(joined) || /[a-z]+-[a-z]+-[a-z]+/i.test(domain);
    return shorteners.has(domain)
      || /xn--/i.test(domain)
      || (riskyWord && structuralAnomaly);
  });
}
...
function buildPhishingIndicators(text, modelPayload = {}) {
  ...
  const accountAction = hasAny(lower, [
    /\b(?:login|log in|sign in)\b/,
    /\b(?:verify|confirm|validate|re-?activate|restore)\b.{0,36}\b(?:account|identity|credentials?|login|sign[ -]?in)\b/,
    /\b(?:account|identity|credentials?|login|sign[ -]?in)\b.{0,36}\b(?:verify|confirm|validate|re-?activate|restore|update)\b/,
  ]);
  if (accountAction) {
    indicators.push(indicator('credential_request', 'Credential or account action request', 'The message asks for login or account verification activity.', 'Medium'));
  }

  if (hasAny(lower, [/\botp\b/, /\bone[-\s]?time\b/, /\bpassword\b/, /\bpasscode\b/, /\b2fa\b/, /\bsecurity code\b/, /\bauthentication code\b/])) {
    indicators.push(indicator('otp_or_password', 'OTP or password language', 'The message references passwords, codes, or authentication details. Context and sender evidence are required before treating this as malicious.', 'Medium'));
  }
  ...
  if (hasAny(lower, [/\bkyc\b/, /\bblocked\b/, /\bsuspended\b/, /\blocked\b/, /\brestricted\b/, /\blimited\b/, /\bdeactivated\b/, /\baccount hold\b/])) {
    indicators.push(indicator('account_blocking', 'Account restriction language', 'The message mentions blocking, suspension, KYC, or access restrictions. This requires corroborating sender or link evidence before high-risk escalation.', 'Medium'));
  }

  if (hasAny(lower, [/\battachment\b/, /\battached\b/, /\bdownload\b/, /\bopen file\b/, /\bview document\b/, /\bscan qr\b/, /\bqr code\b/])) {
    indicators.push(indicator('attachment_risk', 'Attachment or download reference', 'The message references an attachment, download, or QR code. This is contextual evidence rather than a phishing finding by itself.', 'Low'));
  }

  if (entities.urls.length || entities.domains.length) {
    indicators.push(indicator('external_link', 'External link observed', 'The message contains one or more URLs or domains. Link presence alone is not treated as suspicious.', 'Low'));
  }
  ...
  if (entities.emails.length || entities.phones.length || hasAny(lower, [/\bwhatsapp\b/, /\btelegram\b/, /\bcall now\b/, /\breply yes\b/])) {
    indicators.push(indicator('contact_method', 'External contact method', 'The message includes contact details or references another communication channel. This requires context before it is considered suspicious.', 'Low'));
  }
...
function scorePhishingIndicators(indicators) {
  ...
  const actionable = list.filter((item) => ['Medium', 'High', 'Critical'].includes(normalizeRiskLevel(item.severity)));
  const contextualCount = list.length - actionable.length;
  if (!actionable.length) return roundScore(Math.min(0.18, contextualCount * 0.035));
  const scores = actionable.map((item) => severityScore(item.severity));
  const total = scores.reduce((sum, score) => sum + score, 0);
  const maxSeverity = Math.max(...scores);
  const densityBonus = Math.min(0.1, Math.max(0, actionable.length - 2) * 0.025);
  const cumulative = Math.min(0.28, total * 0.1);
  const contextualBonus = Math.min(0.05, contextualCount * 0.01);
  return roundScore(Math.min(0.95, (maxSeverity * 0.55) + cumulative + densityBonus + contextualBonus));
}

function combinePhishingScores(modelScore, ruleScore) {
  const model = clamp01(modelScore);
  const rules = clamp01(ruleScore);
  let combined = model * 0.78 + rules * 0.22;
...
function phishingDecisionState(modelState, modelScore, ruleScore, decisionScore) {
  ...
  // The provider classifier may enter its phishing band at 0.70, but the
  // product verdict is intentionally more conservative: either the model must
  // be very strong, or independent deterministic evidence must corroborate it.
  if (normalizedState === 'LIKELY_PHISHING' && model >= 0.82) return 'LIKELY_PHISHING';
  if (normalizedState === 'LIKELY_PHISHING' && model >= 0.7 && rules >= 0.45 && decision >= 0.6) return 'LIKELY_PHISHING';
  if (decision >= 0.78 && rules >= 0.62) return 'LIKELY_PHISHING';
