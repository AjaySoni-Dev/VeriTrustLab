...
async function getUserFromRequest(req) {
  ...
  if (!token) {
    throw new SupabaseError(401, 'Sign in to use VeriTrust Lab scans.');
  }
...
async function ensureDefaultWorkspace(user, profile) {
  ...
  if (!organization) {
    throw new SupabaseError(500, 'Unable to create a VeriTrust Lab workspace for this user.');
  }
...
async function getProfileContext(req, preferredOrgId = null) {
  ...
  if (!orgId) {
    ...
    if (!repaired) {
      throw new SupabaseError(403, 'No VeriTrust Lab workspace is available for this user.');
    }
  ...
  if (!membership || !organization) {
    ...
    throw new SupabaseError(403, 'You do not have access to this VeriTrust Lab workspace.');
  }
...
async function dashboardSnapshot(req, options = {}) {
  ...
  try {
    ...
    console.error('VeriTrust Lab gateway dashboard history unavailable', {
...
async function runScanLifecycle(context, options, operation) {
  ...
  try {
    ...
    try {
      ...
      console.error('VeriTrust Lab atomic scan failure finalization failed', {
        scan_id: scanId,
...
async function recentScans(context, limit = 20) {
  ...
  try {
    ...
    console.error('VeriTrust Lab gateway scan history unavailable', {
...
const EMAIL_CASE_INPUT_MODES = new Set(['plain_text', 'raw_eml', 'trusted_receiver_event']);
const EMAIL_CASE_SOURCES = new Set(['phishing-v2', 'trusted-receiver-v2']);

function isEmailInvestigationScan(scan = {}) {
  const metadata = scan.metadata || {};
  const inputMetadata = (Array.isArray(scan.scan_inputs) ? scan.scan_inputs : [])
    .map((item) => item?.metadata || {})
    .find((item) => item.input_mode || item.evidence_schema || item.source) || {};
  const inputMode = String(metadata.input_mode || inputMetadata.input_mode || '').toLowerCase();
  const evidenceSchema = String(metadata.evidence_schema || inputMetadata.evidence_schema || '').toLowerCase();
  const source = String(scan.source || metadata.source || metadata.gateway?.source || inputMetadata.source || '').toLowerCase();
  return String(scan.scan_type || '').toLowerCase() === 'phishing'
    || evidenceSchema === 'phishing-evidence-7'
    || EMAIL_CASE_INPUT_MODES.has(inputMode)
    || EMAIL_CASE_SOURCES.has(source);
}
...
async function listCases(context, options = {}) {
  ...
  const gatewayScanIds = rows.map((row) => row.gateway_scan_id).filter(Boolean);
  const [standardScans, gatewayScans] = await Promise.all([
    standardScanIds.length
      ? supabaseFetch(`/rest/v1/scans?id=in.(${standardScanIds.map(eq).join(',')})&select=id,scan_type,source,metadata,scan_inputs(input_kind,metadata)`, { service: true })
      : Promise.resolve([]),
    gatewayScanIds.length
      ? supabaseFetch(`/rest/v1/gateway_scans?id=in.(${gatewayScanIds.map(eq).join(',')})&select=id,source,metadata`, { service: true })
      : Promise.resolve([]),
  ]);
  const emailStandardScanIds = new Set((standardScans || [])
    .filter((scan) => isScanTypeEnabled(logicalScanType(scan)) && isEmailInvestigationScan(scan))
    .map((scan) => scan.id));
  const emailGatewayScanIds = new Set((gatewayScans || [])
    .filter(isEmailInvestigationScan)
    .map((scan) => scan.id));
  rows = rows.filter((row) => (row.standard_scan_id && emailStandardScanIds.has(row.standard_scan_id))
    || (row.gateway_scan_id && emailGatewayScanIds.has(row.gateway_scan_id)));
...
async function getCase(context, caseId) {
  ...
  if (caseRow.gateway_scan_id) {
    const gatewayScans = await supabaseFetch(`/rest/v1/gateway_scans?id=eq.${eq(caseRow.gateway_scan_id)}&select=id,source,metadata&limit=1`, { service: true });
    if (!gatewayScans?.[0] || !isEmailInvestigationScan(gatewayScans[0])) {
      throw new SupabaseError(404, 'Case was not found.', null, 'CASE_NOT_FOUND');
    }
  }
  if (caseRow.standard_scan_id) {
    const scans = await supabaseFetch(`/rest/v1/scans?id=eq.${eq(caseRow.standard_scan_id)}&select=id,scan_type,source,metadata,scan_inputs(input_kind,metadata)&limit=1`, { service: true });
    if (!scans?.[0] || !isScanTypeEnabled(logicalScanType(scans[0])) || !isEmailInvestigationScan(scans[0])) {
      throw new SupabaseError(404, 'Case was not found.', null, 'CASE_NOT_FOUND');
    }
  }
