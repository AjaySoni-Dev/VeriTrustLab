...
const ACTIONABLE_ATTRIBUTE_URL_PATTERN = /\b(?:href|action|formaction)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s>]+))/giu;
...
function extractUrls(text, html = '', maxUrls = MAX_EXTRACTED_URLS) {
  ...
  for (const match of String(html || '').matchAll(ACTIONABLE_ATTRIBUTE_URL_PATTERN)) {
    const raw = decodeBasicEntities(match[1] || match[2] || match[3] || '');
    if (/^https?:\/\//iu.test(raw)) add(raw, 'html_attribute', { attribute_match: match[0].slice(0, 64) });
  }
