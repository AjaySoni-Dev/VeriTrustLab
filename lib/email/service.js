...
const { enrichInfrastructure, extractInfrastructure, hasInfrastructureContext, summarizeInfrastructure } = require('./infrastructure');
...
const { createEvidencePassport, matchPersistedPassport } = require('./evidence-passport');
const { investigationLineage, resolveLineageParent } = require('./lineage');
...
function infrastructureStateFromHops(hops) {
  ...
  const enriched = publicHops.filter(hasInfrastructureContext).length;
  if (enriched === publicHops.length) return 'COMPLETED';
  if (enriched > 0) return 'PARTIAL';
  return 'UNAVAILABLE';
}

function attachmentMetadataCoverage(input) {
  if (input.mode === 'plain_text') return 'UNAVAILABLE';
  const parseState = String(input.parseState || '').toUpperCase();
  const limitations = Array.isArray(input.parserLimitations) ? input.parserLimitations.map((value) => String(value)) : [];
  const parserIncomplete = ['FAILED', 'FAILED_TIMEOUT', 'MALFORMED_LIMIT', 'UNSUPPORTED_LIMIT', 'PARTIAL_LIMIT'].includes(parseState);
  const attachmentLimit = limitations.some((code) => code === 'MIME_PART_LIMIT_REACHED'
    || code === 'DECODED_BYTES_LIMIT_REACHED'
    || /^ATTACHMENT_\d+_LIMIT_REACHED$/u.test(code));
  return parserIncomplete || attachmentLimit ? 'LIMITED' : 'CHECKED';
}
...
function evidenceCompleteness(input) {
  ...
  const dimensions = {
    ...
    attachments: attachmentMetadataCoverage(input),
  ...
  if (input.mode === 'plain_text') {
    nextActions.push({
      ...
      label: 'Upload a raw .eml export to add message-carried authentication/identity, MIME, attachment metadata/hash, and recorded relay evidence',
      target_mode: 'raw_eml',
...
function evidenceBundle(input) {
  return {
    ...
    investigation_lineage: input.lineage || investigationLineage({ parentScanId: input.parentScanId || null, currentScanId: input.scanId || null, mode: input.mode }),
...
async function analyzeEmail(input) {
  let scanId = null;
  const rawProgress = input.onProgress || (() => {});
  const progress = (stage, state, message) => rawProgress(stage, state, message, scanId ? { scan_id: scanId } : null);
  ...
  const lineageParent = await resolveLineageParent({
  ...
  const submitted = await submitScan({
    ...
    metadata: { input_mode: mode, evidence_schema: EMAIL_EVIDENCE_SCHEMA, pipeline_version: EMAIL_PIPELINE_VERSION, parent_scan_id: input.parentScanId || null, parent_acquisition_stage: lineageParent?.parentMode || null, lineage_relationship: input.parentScanId ? 'user_linked_evidence_upgrade' : null },
  });
  ...
  scanId = submitted.scan_id;
  const lineage = investigationLineage({
    parentScanId: input.parentScanId || null,
    currentScanId: scanId,
    mode,
    parentMode: lineageParent?.parentMode || null,
  });
  ...
  try {
    ...
    if (mode !== 'plain_text') {
      progress('storage', 'running', 'Storing the supplied raw email in temporary configured storage.');
      ...
      progress('storage', 'completed', 'Supplied raw email stored and retention cleanup scheduled.');
    }
    ...
    progress('sender', mode === 'plain_text' ? 'skipped' : 'running', mode === 'plain_text' ? 'Authentication and alignment checks require raw message headers or trusted receiver facts.' : 'Evaluating email authentication and sender alignment.');
    ...
    const infrastructureProgressState = !infrastructureBase.hops.length
      ? 'skipped'
      : (infrastructureResult.state === 'UNAVAILABLE' ? 'failed' : 'completed');
    const infrastructureProgressMessage = !infrastructureBase.hops.length
      ? 'No eligible mail-server hops are available for lookup.'
      : (infrastructureResult.state === 'COMPLETED'
        ? 'Mail-server infrastructure enrichment finished.'
        : (infrastructureResult.state === 'PARTIAL'
          ? 'Mail-server infrastructure enrichment finished with explicit lookup limitations.'
          : 'Mail-server infrastructure enrichment was unavailable; relay provenance is retained without fabricated location data.'));
    progress('infrastructure', infrastructureProgressState, infrastructureProgressMessage);
    ...
    try {
      ...
      if (matches.available && matches.truncated) {
        campaignMemory = {
          ...campaignMemory,
          state: campaignMemory.state === 'CORRELATED' ? 'CORRELATED' : 'PARTIAL',
          limitation: matches.limitation || 'CAMPAIGN_MEMORY_MATCH_SET_TRUNCATED',
          search_truncated: true,
        };
        campaignSchemaLimitation = matches.limitation || 'CAMPAIGN_MEMORY_MATCH_SET_TRUNCATED';
      }
    ...
    progress('campaign', campaignMemory.state === 'UNAVAILABLE' ? 'failed' : 'completed', campaignMemory.state === 'CORRELATED'
      ? `Campaign Memory linked ${campaignMemory.related_scan_count} prior investigation(s).${campaignMemory.search_truncated ? ' The bounded historical search was truncated and is recorded as a limitation.' : ''}`
      : (campaignMemory.state === 'PARTIAL'
        ? 'Campaign Memory historical lookup reached its bounded search limit; no definitive no-match claim is made.'
        : (campaignMemory.state === 'UNAVAILABLE' ? 'Campaign Memory is unavailable; the scan remains valid and records the limitation.' : 'No strong prior correlation match was found.')));
    ...
    await Promise.all([
      publishDecision(scanId, decision, auth.user?.id || null).catch((error) => {
        console.error(JSON.stringify({ timestamp: now(), level: 'warn', service: 'veritrust-email', event: 'decision.publish_failed', scan_id: scanId, code: String(error?.code || 'DECISION_PUBLISH_FAILED') }));
        return null;
      }),
      completeArtifacts(auth.organization.id, scanId).catch((error) => {
        console.error(JSON.stringify({ timestamp: now(), level: 'warn', service: 'veritrust-email', event: 'decision.artifacts_complete_failed', scan_id: scanId, code: String(error?.code || 'ARTIFACTS_COMPLETE_FAILED') }));
        return null;
      }),
    ]);
    ...
    const bundle = evidenceBundle({
      ...
      parserLimitations: parsed.limitations,
    ...
    await storeIdempotentResponse(scanId, 200, body).catch((error) => {
      console.error(JSON.stringify({ timestamp: now(), level: 'warn', service: 'veritrust-email', event: 'decision.store_response_failed', scan_id: scanId, code: String(error?.code || 'STORE_RESPONSE_FAILED') }));
      return null;
    });
...
async function emailEvidenceReport(orgId, scanId) {
  ...
  const report = {
    ...
    evidence_completeness: evidenceCompleteness({
      ...
      parseState: records.details.parse_state,
      parserLimitations: records.details.limitations || [],
    }),
    ...
    investigation_lineage: (() => {
      const stored = records.details.metadata?.investigation_lineage || {};
      const parentId = stored.parent_scan_id || gateway.scan.metadata?.parent_scan_id || null;
      return {
        ...investigationLineage({ parentScanId: parentId, currentScanId: scanId, mode, parentMode: stored.parent_acquisition_stage || gateway.scan.metadata?.parent_acquisition_stage || null }),
        ...stored,
        relationship: parentId ? (stored.relationship || 'user_linked_evidence_upgrade') : null,
        same_message_verified: parentId ? Boolean(stored.same_message_verified) : null,
      };
    })(),
  ...
  const persistedPassport = matchPersistedPassport(report, records.passport);
  if (persistedPassport.matched) {
    report.evidence_passport = persistedPassport.passport;
  } else {
    report.evidence_passport = null;
    report.limitations = [...new Set([...(report.limitations || []), 'SIGNED_EVIDENCE_SNAPSHOT_UNAVAILABLE'])];
  }
  return report;
}

module.exports = { analyzeEmail, attachmentMetadataCoverage, emailEvidenceReport, evidenceBundle, evidenceCompleteness, evidenceManifest, infrastructureStateFromHops, normalizeEmailObservation, requestHash, specialistState };
