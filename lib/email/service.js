...
const { invokeDetectionModule } = require('../module-communication');
...
async function runPreparedLinkBatch(orgId, scanId, artifacts, policy) {
  ...
  if (runnable.length) {
    const invocationSlots = await mapWithConcurrency(runnable, serverConfig.linkAnalysisConcurrency, async (slot, index) => {
      const timeoutMs = modelTimeoutMs(policy, slot.prepared, serverConfig.gatewaySynchronousBudgetMs);
      try {
        const invocation = await invokeDetectionModule('link', slot.artifact, {
          modelKey: 'swift',
          required: true,
          timeoutMs,
          allowProviderFallback: false,
          modelContract: slot.prepared.configuration,
          correlationId: `${scanId}:mailgraph:swift:${index}`,
        });
        return { slot, result: invocation.result };
      } catch (error) {
        return { slot, error };
      }
    });
    await mapWithConcurrency(invocationSlots, 8, async ({ slot, result, error }) => {
      if (error) {
        evidenceByArtifact.set(slot.artifact.id, failedModelEvidence('link', 'swift', slot.artifact.id, error, true));
        return;
      }
      try {
        ...
        evidenceByArtifact.set(slot.artifact.id, failedModelEvidence('link', 'swift', slot.artifact.id, recordError, true));
      }
    });
  }
...
function evidenceCompleteness(input) {
  ...
  const urlStates = urlChildren.map((child) => input.childEvidence?.get?.(child.id || child.artifact_id)?.status || child.state || 'unavailable');
  ...
  const linkAnalysis = {
    extracted: urlChildren.length,
    completed: urlStates.filter((state) => state === 'completed').length,
    failed: urlStates.filter((state) => state === 'failed').length,
    timed_out: urlStates.filter((state) => state === 'timed_out').length,
    pending: urlStates.filter((state) => ['pending', 'queued', 'leased', 'running', 'unavailable'].includes(state)).length,
    all_completed: !urlChildren.length || urlStates.every((state) => state === 'completed'),
  };
  return {
    level,
...
function evidenceBundle(input) {
  return {
    ...
    children: input.children.map((child) => ({
      ...
      score: input.childEvidence.get(child.id)?.score ?? null,
      verdict: input.childEvidence.get(child.id)?.verdict || 'unknown',
      confidence: input.childEvidence.get(child.id)?.confidence || 'unknown',
...
async function analyzeEmail(input) {
  ...
  try {
    ...
    const decision = correlate(correlationInputs, policy, { emailContext: true });
...
async function emailEvidenceReport(orgId, scanId) {
  ...
  const children = (gateway.artifacts || []).filter((item) => item.parent_artifact_id === parentArtifactId).map((child) => {
    ...
    return {
      ...
      score: childEvidence?.score ?? null,
      verdict: childEvidence?.verdict || 'unknown',
      confidence: childEvidence?.confidence || 'unknown',
  ...
  const report = {
    ...
    evidence_completeness: evidenceCompleteness({
      ...
      children,
      childEvidence: new Map((gateway.evidence || []).map((item) => [item.artifact_id, { status: item.status, score: item.score, verdict: item.verdict, confidence: item.confidence, reasonCodes: item.reason_codes || [] }])),
