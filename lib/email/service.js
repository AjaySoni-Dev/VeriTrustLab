...
const { createEvidencePassport, trustedEvidenceKeyIds, verifyEvidencePackage } = require('./evidence-passport');
const {
  assertStoredPackageIntegrity,
  reconstructedIntegrity,
  resolveHistoricalEvidencePackage,
  storedPassportFieldsMatch,
} = require('./evidence-history');
...
async function analyzeEmail(input) {
  ...
  try {
    ...
    const selfVerification = verifyEvidencePackage({ passport, evidence: bundle }, { trustedKeyIds: trustedEvidenceKeyIds() });
    if (!selfVerification.valid) {
      const error = new Error('New Evidence Passport failed complete self-verification and was not persisted.');
      error.code = 'EVIDENCE_PASSPORT_SELF_VERIFY_FAILED';
      error.verification = selfVerification;
      throw error;
    }
    const persistedPassport = await persistEvidencePassport(auth.organization.id, scanId, artifact.id, passport, bundle);
    if (persistedPassport.available === false || !persistedPassport.row) {
      const error = new Error('Evidence Passport durable storage is not available; the investigation cannot be completed with a signed package.');
      error.code = persistedPassport.limitation || 'EVIDENCE_PASSPORT_PERSISTENCE_UNAVAILABLE';
      throw error;
    }
    // Do not mutate evidence after the exact signed payload has been committed. The
    // envelope is attached only after self-verification and conflict-safe persistence.
...
async function emailEvidenceReport(orgId, scanId, options = {}) {
  const scanReportFn = options.scanReport || scanReport;
  const emailRecordsForScanFn = options.emailRecordsForScan || emailRecordsForScan;
  const [gateway, records] = await Promise.all([scanReportFn(orgId, scanId), emailRecordsForScanFn(orgId, scanId)]);
  if (!records.details) return null;

  const historical = resolveHistoricalEvidencePackage({
    orgId,
    scanId,
    artifactId: records.details.artifact_id,
    passportRow: records.passport,
    storedEvidence: gateway.scan?.response_body?.evidence || null,
    passportSchemaAvailable: records.passport_schema_available,
  });
  if (historical.evidence) return historical.evidence;
  ...
  return {
    ...
    evidence_integrity: historical.evidence_integrity,
  };
}

module.exports = { analyzeEmail, assertStoredPackageIntegrity, emailEvidenceReport, evidenceBundle, evidenceCompleteness, evidenceManifest, infrastructureStateFromHops, normalizeEmailObservation, reconstructedIntegrity, requestHash, specialistState, storedPassportFieldsMatch };
