...
function campaignFromMatches(currentScanId, currentEntities, matches) {
  ...
  return {
    ...
    scoring_notice: 'Campaign Memory uses deterministic weighted overlap. An exact attachment hash can correlate by itself; otherwise at least two independent entity types and sufficient combined weight are required. ASN-only, IP-only, sender-domain-only, or URL-domain-only overlap cannot create a correlation match.',
    producer_version: CAMPAIGN_MEMORY_VERSION,
  };
}
