...
async function parseEml(rawInput, options = {}) {
  ...
  const parsed = await new Promise((resolve, reject) => {
    ...
    parser.on('data', (part) => {
      ...
      if (mimePartCount > MAX_MIME_PARTS) {
        ...
        if (!limitations.includes('MIME_PART_LIMIT_REACHED')) limitations.push('MIME_PART_LIMIT_REACHED');
        // MailParser requires attachment streams to be consumed/released. Past the
        // evidence budget we drain without hashing, recording, or retaining data.
        if (part.type === 'attachment' && part.content) {
          let released = false;
          const release = () => {
            if (released) return;
            released = true;
            try { part.release(); } catch { /* stream cleanup only */ }
          };
          part.content.once('error', release);
          part.content.once('end', release);
          part.content.resume();
        }
        return;
      }
  ...
  return {
    ...
    processedMimePartCount: Math.min(mimePartCount, MAX_MIME_PARTS),
    skippedMimeParts,
