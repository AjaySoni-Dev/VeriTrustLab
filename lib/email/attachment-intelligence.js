...
function analyzeAttachmentMetadata(attachment = {}) {
  ...
  if (EXECUTABLE_OR_SCRIPT.has(extension)) flags.push({ code: 'EXECUTABLE_OR_SCRIPT_EXTENSION', severity: 'HIGH', explanation: 'The attachment uses an executable or script-oriented extension. VeriTrust Lab does not execute it.' });
  ...
  if (ARCHIVE.has(extension)) flags.push({ code: 'ARCHIVE_ATTACHMENT', severity: 'INFO', explanation: 'The attachment is an archive; its contents are not executed by VeriTrust Lab.' });
