...
function receivedSourceSegment(line) {
  const value = String(line || '');
  const from = /\bfrom\b/iu.exec(value);
  if (!from) return '';
  const remainder = value.slice(from.index + from[0].length);
  const by = /\s+\bby\b/iu.exec(remainder);
  const semicolon = remainder.indexOf(';');
  const boundaries = [by?.index, semicolon].filter((index) => Number.isInteger(index) && index >= 0);
  const end = boundaries.length ? Math.min(...boundaries) : remainder.length;
  return remainder.slice(0, end);
}

function sourceCandidateIps(line) {
  return candidateIps(receivedSourceSegment(line));
}
...
function extractInfrastructure(headerLines, options = {}) {
  ...
  const hops = received.map((row, index) => {
    ...
    const sourceIps = sourceCandidateIps(line);
    const selected = sourceIps.find((ip) => classifyIp(ip) === 'public') || sourceIps[0] || null;
    ...
    return {
      ...
      provenance: { source: 'received_header', all_candidate_ips: ips, source_candidate_ips: sourceIps, selected_from_source_segment: Boolean(selected), exact_location_claim: false },
...
module.exports = { candidateIps, classifyIp, enrichInfrastructure, extractInfrastructure, receivedSourceSegment, sourceCandidateIps, summarizeInfrastructure };
