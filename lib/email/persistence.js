...
async function persistEmailDetails(orgId, scanId, artifactId, parsed, identity, urlCount, extraMetadata = {}) {
  ...
  const body = {
    ...
    metadata: {
      ...
      processed_mime_part_count: parsed.processedMimePartCount ?? Math.min(parsed.mimePartCount || 0, 100),
      skipped_mime_parts: parsed.skippedMimeParts || 0,
...
async function matchingThreatEntities(orgId, scanId, entities) {
  ...
  if (!hashes.length) return { available: true, rows: [], truncated: false };
  const pageSize = 500;
  const maxRows = 5000;
  try {
    const rows = [];
    for (let offset = 0; offset < maxRows; offset += pageSize) {
      const page = await supabaseFetch(`/rest/v1/email_threat_entities?org_id=eq.${eq(orgId)}&scan_id=neq.${eq(scanId)}&value_hash=in.(${hashes.join(',')})&select=scan_id,entity_type,entity_value,value_hash,weight,trust_level,created_at&order=created_at.desc&limit=${pageSize}&offset=${offset}`, { service: true });
      rows.push(...(page || []));
      if (!page || page.length < pageSize) return { available: true, rows, truncated: false };
    }
    return { available: true, rows, truncated: true, limitation: 'CAMPAIGN_MEMORY_MATCH_SET_TRUNCATED' };
  } catch (error) {
    if (isOptionalSchemaMissing(error)) return { available: false, rows: [], truncated: false, limitation: 'CAMPAIGN_MEMORY_SCHEMA_NOT_APPLIED' };
    throw error;
  }
}
