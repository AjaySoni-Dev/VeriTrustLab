...
function passportPayload({ scanId, evidence, decision = null }) {
  ...
  return {
    ...
    statement: 'A successful full-package verification checks the signature, signed evidence and manifest hashes, key identity, and configured issuer trust. It does not establish content truth, sender identity, message safety, attribution, or legal admissibility.',
  };
}
...
function passportFromPersistenceRow(row) {
  ...
  return {
    ...payload,
    signature_algorithm: row.signature_algorithm || SIGNATURE_ALGORITHM,
    key_id: row.key_id || null,
    public_key_jwk: row.public_key_jwk || null,
    signature: row.signature || null,
  };
}

function matchPersistedPassport(evidence, row) {
  const passport = passportFromPersistenceRow(row);
  if (!passport) return { matched: false, passport: null, reason: 'PERSISTED_PASSPORT_UNAVAILABLE' };
  try {
    const normalizedEvidence = evidenceForDigest(evidence);
    const evidenceHash = sha256(canonicalize(normalizedEvidence));
    const manifestHash = sha256(canonicalize(evidence?.evidence_manifest || {}));
    const expectedEvidenceHash = String(row.evidence_sha256 || passport.evidence_sha256 || '');
    const expectedManifestHash = String(row.manifest_sha256 || passport.manifest_sha256 || '');
    const matched = Boolean(expectedEvidenceHash && expectedManifestHash
      && evidenceHash === expectedEvidenceHash
      && manifestHash === expectedManifestHash);
    return {
      matched,
      passport: matched ? passport : null,
      reason: matched ? null : 'SIGNED_EVIDENCE_SNAPSHOT_UNAVAILABLE',
    ...
    return {
      matched: false,
      passport: null,
      reason: error?.code || 'SIGNED_EVIDENCE_SNAPSHOT_UNAVAILABLE',
    };
  }
}
