...
const DEEPFAKE_MODELS = {
  pixel: {
    key: 'pixel',
    display_name: 'VeriTrust Lab Pixel',
    model_path_key: 'deepfake_pixel',
    provider: 'hf-inference',
  },
  prism: {
    key: 'prism',
    display_name: 'VeriTrust Lab Prism',
...
const PHISHING_MODELS = {
  mailguard: {
    key: 'mailguard',
    display_name: 'VeriTrust Lab MailGuard',
    model_path_key: 'phishing_mailguard',
    provider: 'hf-inference',
  },
  cortex: {
    key: 'cortex',
    display_name: 'VeriTrust Lab Cortex',
  ...
  if (!applyCors(req, res)) {
    sendJson(res, 403, {
      ...
      error: {
        ...
        message: 'This origin is not allowed to access the VeriTrust Lab API.',
      },
    });
    return true;
  }
...
function logApiError(error, status, code) {
  ...
  console.error('VeriTrust Lab API error', {
    code,
    status,
    name: error?.name,
